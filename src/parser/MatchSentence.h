...
class MatchEdgeProp final {
 public:
  MatchEdgeProp(std::string* alias, MatchEdgeTypeList* types, MatchStepRange* range, Expression* props = nullptr) {
    alias_.reset(alias);
    range_.reset(range);
  ...
  auto get() && { return std::make_tuple(std::move(alias_), std::move(types_), std::move(range_), std::move(props_)); }
...
class MatchNodeLabel final {
 public:
  ...
  MapExpression* props() { return props_.get(); }
...
class MatchReturn final {
 public:
  explicit MatchReturn(YieldColumns* columns = nullptr, OrderFactors* orderFactors = nullptr,
                       Expression* skip = nullptr, Expression* limit = nullptr, bool distinct = false) {
  ...
  void setColumns(YieldColumns* columns) { columns_.reset(columns); }

  bool isAll() const { return isAll_; }

  bool isDistinct() const { return isDistinct_; }

  const Expression* skip() const { return skip_.get(); }
...
class UnwindClause final : public ReadingClause {
 public:
  ...
  std::string* alias() { return alias_.get(); }
...
class WithClause final : public ReadingClause {
 public:
  explicit WithClause(YieldColumns* cols, OrderFactors* orderFactors = nullptr, Expression* skip = nullptr,
                      Expression* limit = nullptr, WhereClause* where = nullptr, bool distinct = false)
  ...
  Expression* skip() { return skip_.get(); }

  const Expression* skip() const { return skip_.get(); }

  Expression* limit() { return limit_.get(); }
