...
class InsertVerticesSentence final : public Sentence {
 public:
  InsertVerticesSentence(VertexTagList *tagList, VertexRowList *rows, bool overwritable = true) {
...
class InsertEdgesSentence final : public Sentence {
 public:
  InsertEdgesSentence() { kind_ = Kind::kInsertEdges; }
...
class UpdateItem final {
 public:
  ...
  const Expression *getFieldExpr() const { return fieldExpr_.get(); }
...
class UpdateBaseSentence : public Sentence {
 public:
  UpdateBaseSentence(UpdateList *updateList, WhenClause *whenClause, YieldClause *yieldClause, std::string *name,
...
class UpdateVertexSentence final : public UpdateBaseSentence {
 public:
  UpdateVertexSentence(Expression *vid, std::string *tagName, UpdateList *updateList, WhenClause *whenClause,
                       YieldClause *yieldClause, bool isInsertable = false)
      : UpdateBaseSentence(updateList, whenClause, yieldClause, tagName, isInsertable) {
    kind_ = Kind::kUpdateVertex;
    vid_.reset(vid);
  }

  UpdateVertexSentence(Expression *vid, UpdateList *updateList, WhenClause *whenClause, YieldClause *yieldClause,
                       bool isInsertable = false)
  ...
  Expression *getVid() const { return vid_.get(); }
...
class UpdateEdgeSentence final : public UpdateBaseSentence {
 public:
  UpdateEdgeSentence(Expression *srcId, Expression *dstId, int64_t rank, std::string *edgeName, UpdateList *updateList,
                     WhenClause *whenClause, YieldClause *yieldClause, bool isInsertable = false)
  ...
  Expression *getSrcId() const { return srcId_.get(); }

  Expression *getDstId() const { return dstId_.get(); }
  ...
  void setUrl(std::string &url) {
    ...
    if (tokens.size() == 2) {
      ...
      if (position != -1) {
        ...
        path_ = std::make_unique<std::string>(tokens[1].toString().substr(position, tokens[1].size()));
      } else {
