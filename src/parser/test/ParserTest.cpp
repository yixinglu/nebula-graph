...
TEST(Parser, InsertEdge) {
  ...
  {
    std::string query =
        "INSERT EDGE NO OVERWRITE transfer(amount, time_) "
        "VALUES \"-12345\"->\"54321\":(3.75, 1537408527)";
    auto result = parse(query);
    ASSERT_TRUE(result.ok()) << result.status();
  }
  ...
  {
    std::string query =
        "INSERT EDGE NO OVERWRITE transfer() "
        "VALUES \"12345\"->\"54321@1537408527\":()";
    auto result = parse(query);
    ASSERT_TRUE(result.ok()) << result.status();
  }
...
TEST(Parser, MatchMultipleTags) {
  ...
  {
    std::string query = "MATCH (:person {name: 'Tom'}:player {id: 233}) --> (:person {name: 'Jerry'}) RETURN *";
    auto result = parse(query);
    ASSERT_TRUE(result.ok()) << result.status();
  }
  {
    std::string query = "MATCH (:person:player {id: 233}) --> (:person {name: 'Jerry'}) RETURN *";
    auto result = parse(query);
    ASSERT_TRUE(result.ok()) << result.status();
  }
  {
