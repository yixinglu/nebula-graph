...
template <typename T>
static std::enable_if_t<std::is_integral<T>::value, AssertionResult> checkSemanticValue(T expected, semantic_type *sv) {
...
template <typename T>
static std::enable_if_t<std::is_floating_point<T>::value, AssertionResult> checkSemanticValue(T expected,
                                                                                              semantic_type *sv) {
...
#define CHECK_SEMANTIC_TYPE(STR, TYPE)                                                                              \
  (stream += " ", stream += STR, [&]() {                                                                            \
    ...
    if (actual != TYPE) {                                                                                           \
      return AssertionFailure() << "Token type not match for `" << STR << "', expected: " << static_cast<int>(TYPE) \
