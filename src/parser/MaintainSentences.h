...
class ColumnSpecification final {
 public:
  ColumnSpecification(std::string *name, meta::cpp2::PropertyType type, bool isNull, Expression *defaultVaule,
                      int16_t typeLen = 0) {
    name_.reset(name);
    type_ = type;
    isNull_ = isNull;
...
class CreateTagSentence final : public CreateSentence {
 public:
  CreateTagSentence(std::string *name, ColumnSpecificationList *columns, SchemaPropList *schemaProps, bool ifNotExists)
...
class CreateEdgeSentence final : public CreateSentence {
 public:
  CreateEdgeSentence(std::string *name, ColumnSpecificationList *columns, SchemaPropList *schemaProps, bool ifNotExists)
      : CreateSentence(ifNotExists) {
    name_.reset(name);
    columns_.reset(columns);
    schemaProps_.reset(schemaProps);
...
class IndexFieldList final {
 public:
  ...
  void addField(std::unique_ptr<meta::cpp2::IndexFieldDef> field) { fields_.emplace_back(std::move(field)); }
...
class CreateTagIndexSentence final : public CreateSentence {
 public:
  CreateTagIndexSentence(std::string *indexName, std::string *tagName, IndexFieldList *fields, bool ifNotExists)
      : CreateSentence(ifNotExists) {
...
class CreateEdgeIndexSentence final : public CreateSentence {
 public:
  CreateEdgeIndexSentence(std::string *indexName, std::string *edgeName, IndexFieldList *fields, bool ifNotExists)
      : CreateSentence(ifNotExists) {
    indexName_.reset(indexName);
...
class ShowTagIndexesSentence : public Sentence {
 public:
  ShowTagIndexesSentence() { kind_ = Kind::kShowTagIndexes; }
...
class ShowEdgeIndexesSentence : public Sentence {
 public:
  ShowEdgeIndexesSentence() { kind_ = Kind::kShowEdgeIndexes; }
