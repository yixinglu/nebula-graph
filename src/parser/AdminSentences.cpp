...
std::string SetConfigSentence::toString() const { return std::string("UPDATE CONFIGS ") + configItem_->toString(); }

std::string GetConfigSentence::toString() const { return std::string("GET CONFIGS ") + configItem_->toString(); }
    ...
    case meta::cpp2::AdminJobOp::ADD: {
      switch (cmd_) {
        ...
        case meta::cpp2::AdminCmd::FLUSH:
          return paras_.empty() ? "SUBMIT JOB FLUSH" : folly::stringPrintf("SUBMIT JOB FLUSH %s", paras_[0].c_str());
        ...
        case meta::cpp2::AdminCmd::STATS:
          return paras_.empty() ? "SUBMIT JOB STATS" : folly::stringPrintf("SUBMIT JOB STATS %s", paras_[0].c_str());
        case meta::cpp2::AdminCmd::DOWELOAD:
          return paras_.empty() ? "DOWELOAD HDFS " : folly::stringPrintf("DOWELOAD HDFS %s", paras_[0].c_str());
        case meta::cpp2::AdminCmd::INGEST:
          return "INGEST";
        case meta::cpp2::AdminCmd::DATA_BALANCE:
        case meta::cpp2::AdminCmd::UNKNOWN:
          return folly::stringPrintf("Unsupported AdminCmd: %s", apache::thrift::util::enumNameSafe(cmd_).c_str());
      }
    }
...
void AdminJobSentence::addPara(const NameLabelList &paras) {
  ...
  std::for_each(labels.begin(), labels.end(), [this](const auto &para) { paras_.emplace_back(*para); });
}
