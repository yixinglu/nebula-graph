...
class GoSentence final : public Sentence {
 public:
  GoSentence() { kind_ = Kind::kGo; }

  void setStepClause(StepClause* clause) { stepClause_.reset(clause); }

  void setFromClause(FromClause* clause) { fromClause_.reset(clause); }

  void setOverClause(OverClause* clause) { overClause_.reset(clause); }
...
class LookupSentence final : public Sentence {
 public:
  ...
  const std::string* from() const { return from_.get(); }

  void setWhereClause(WhereClause* whereClause) { whereClause_.reset(whereClause); }
...
class OrderFactor final {
 public:
  ...
  void setExpr(Expression* expr) { expr_.reset(expr); }
...
class FindPathSentence final : public Sentence {
 public:
  ...
  bool withProperites() const { return withProperites_; }
...
class LimitSentence final : public Sentence {
 public:
  explicit LimitSentence(int64_t offset, int64_t count) : offset_(offset), count_(count) { kind_ = Kind::kLimit; }
...
class GroupBySentence final : public Sentence {
 public:
  explicit GroupBySentence(YieldClause* yield = nullptr, WhereClause* having = nullptr,
...
class GetSubgraphSentence final : public Sentence {
 public:
  GetSubgraphSentence(StepClause* step, FromClause* from, InBoundClause* in, OutBoundClause* out,
