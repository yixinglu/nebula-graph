...
class ShowHostsSentence : public Sentence {
 public:
  ...
  explicit ShowHostsSentence(meta::cpp2::ListHostType type) : type_(type) { kind_ = Kind::kShowHosts; }
...
class SpaceOptItem final {
 public:
  ...
  enum OptionType : uint8_t { PARTITION_NUM, REPLICA_FACTOR, VID_TYPE, CHARSET, COLLATE, ATOMIC_EDGE, GROUP_NAME };
  ...
  const meta::cpp2::ColumnTypeDef& asTypeDef() const { return boost::get<meta::cpp2::ColumnTypeDef>(optValue_); }
...
class ShowConfigsSentence final : public ConfigBaseSentence {
 public:
  explicit ShowConfigsSentence(ConfigRowItem* item) : ConfigBaseSentence(Kind::kShowConfigs, item) {}
...
class AdminJobSentence final : public Sentence {
 public:
  explicit AdminJobSentence(meta::cpp2::AdminJobOp op, meta::cpp2::AdminCmd cmd = meta::cpp2::AdminCmd::UNKNOWN)
      : op_(op), cmd_(cmd) {
