...
class StepClause final {
 public:
  ...
  explicit StepClause(uint32_t steps = 1) { steps_ = steps; }
  ...
  MToN *mToN() const { return mToN_.get(); }

  bool isMToN() const { return mToN_ != nullptr; }
...
class VerticesClause {
 public:
  ...
  explicit VerticesClause(Expression *ref) { ref_.reset(ref); }

  auto vidList() const { return vidList_->vidList(); }

  auto isRef() const { return ref_ != nullptr; }

  auto ref() const { return ref_.get(); }
...
class OverClause final {
 public:
  ...
  explicit OverClause(bool isOverAll, storage::cpp2::EdgeDirection direction = storage::cpp2::EdgeDirection::OUT_EDGE) {
...
class WhereClause {
 public:
  explicit WhereClause(Expression *filter) { filter_.reset(filter); }

  Expression *filter() const { return filter_.get(); }

  void setFilter(Expression *expr) { filter_.reset(expr); }

  std::unique_ptr<WhereClause> clone() const { return std::make_unique<WhereClause>(filter_->clone().release()); }
