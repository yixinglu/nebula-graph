...
class Session final {
 public:
  ...
  const SpaceInfo& space() const { return space_; }

  void setSpace(SpaceInfo space) { space_ = std::move(space); }

  const std::string& user() const { return account_; }

  void setAccount(std::string account) { account_ = std::move(account); }

  std::unordered_map<GraphSpaceID, meta::cpp2::RoleType> roles() const { return roles_; }
  ...
  void setRole(GraphSpaceID space, meta::cpp2::RoleType role) { roles_.emplace(space, role); }
