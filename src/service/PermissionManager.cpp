...
Status PermissionManager::canWriteRole(Session *session, meta::cpp2::RoleType targetRole, GraphSpaceID spaceId,
  ...
  return Status::PermissionError("No permission to grant/revoke `%s' to `%s'.",
                                 apache::thrift::util::enumNameSafe(role).c_str(), targetUser.c_str());
}
