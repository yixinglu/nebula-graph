...
std::unique_ptr<PlanNodeDescription> GetNeighbors::explain() const {
  ...
  addDescription("vertexProps", vertexProps_ ? folly::toJson(util::toJson(*vertexProps_)) : "", desc.get());
  addDescription("edgeProps", edgeProps_ ? folly::toJson(util::toJson(*edgeProps_)) : "", desc.get());
  addDescription("statProps", statProps_ ? folly::toJson(util::toJson(*statProps_)) : "", desc.get());
...
IndexScan* IndexScan::clone(QueryContext* qctx) const {
  auto* scan = IndexScan::make(qctx, nullptr, space(), nullptr, nullptr, isEdge(), schemaId());
  scan->clone(*this);
  return scan;
}
...
Join::Join(QueryContext* qctx, Kind kind, PlanNode* input, std::pair<std::string, int64_t> leftVar,
