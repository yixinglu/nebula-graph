...
class Explore : public SingleInputNode {
 ...
 protected:
  Explore(QueryContext* qctx, Kind kind, PlanNode* input, GraphSpaceID space, bool dedup, int64_t limit,
          std::string filter, std::vector<storage::cpp2::OrderBy> orderBy)
...
class GetNeighbors final : public Explore {
 public:
  ...
  static GetNeighbors* make(QueryContext* qctx, PlanNode* input, GraphSpaceID space, Expression* src,
                            std::vector<EdgeType> edgeTypes, storage::cpp2::EdgeDirection edgeDirection,
                            VertexProps&& vertexProps, EdgeProps&& edgeProps, StatProps&& statProps, Exprs&& exprs,
                            bool dedup = false, bool random = false, std::vector<storage::cpp2::OrderBy> orderBy = {},
                            int64_t limit = -1, std::string filter = "") {
  ...
  const std::vector<storage::cpp2::VertexProp>* vertexProps() const { return vertexProps_.get(); }

  const std::vector<storage::cpp2::EdgeProp>* edgeProps() const { return edgeProps_.get(); }

  const std::vector<storage::cpp2::StatProp>* statProps() const { return statProps_.get(); }

  const std::vector<storage::cpp2::Expr>* exprs() const { return exprs_.get(); }

  bool random() const { return random_; }

  void setSrc(Expression* src) { src_ = src; }

  void setEdgeDirection(storage::cpp2::EdgeDirection direction) { edgeDirection_ = direction; }

  void setEdgeTypes(std::vector<EdgeType> edgeTypes) { edgeTypes_ = std::move(edgeTypes); }

  void setVertexProps(VertexProps vertexProps) { vertexProps_ = std::move(vertexProps); }

  void setEdgeProps(EdgeProps edgeProps) { edgeProps_ = std::move(edgeProps); }

  void setStatProps(StatProps statProps) { statProps_ = std::move(statProps); }

  void setExprs(Exprs exprs) { exprs_ = std::move(exprs); }
...
class GetVertices final : public Explore {
 public:
  static GetVertices* make(QueryContext* qctx, PlanNode* input, GraphSpaceID space, Expression* src,
                           std::vector<storage::cpp2::VertexProp> props, std::vector<storage::cpp2::Expr> exprs,
                           bool dedup = false, std::vector<storage::cpp2::OrderBy> orderBy = {},
                           int64_t limit = std::numeric_limits<int64_t>::max(), std::string filter = "") {
    return qctx->objPool()->add(new GetVertices(qctx, input, space, src, std::move(props), std::move(exprs), dedup,
                                                std::move(orderBy), limit, std::move(filter)));
  }
  ...
  const std::vector<storage::cpp2::VertexProp>& props() const { return props_; }

  const std::vector<storage::cpp2::Expr>& exprs() const { return exprs_; }
 ...
 private:
  ...
  GetVertices(QueryContext* qctx, PlanNode* input, GraphSpaceID space, Expression* src,
              std::vector<storage::cpp2::VertexProp> props, std::vector<storage::cpp2::Expr> exprs, bool dedup,
              std::vector<storage::cpp2::OrderBy> orderBy, int64_t limit, std::string filter)
      : Explore(qctx, Kind::kGetVertices, input, space, dedup, limit, std::move(filter), std::move(orderBy)),
        src_(src),
...
class GetEdges final : public Explore {
 public:
  static GetEdges* make(QueryContext* qctx, PlanNode* input, GraphSpaceID space, Expression* src, Expression* type,
                        Expression* ranking, Expression* dst, std::vector<storage::cpp2::EdgeProp> props,
                        std::vector<storage::cpp2::Expr> exprs, bool dedup = false,
                        int64_t limit = std::numeric_limits<int64_t>::max(),
                        std::vector<storage::cpp2::OrderBy> orderBy = {}, std::string filter = "") {
    return qctx->objPool()->add(new GetEdges(qctx, input, space, src, type, ranking, dst, std::move(props),
                                             std::move(exprs), dedup, limit, std::move(orderBy), std::move(filter)));
  }
  ...
  const std::vector<storage::cpp2::EdgeProp>& props() const { return props_; }

  const std::vector<storage::cpp2::Expr>& exprs() const { return exprs_; }

 private:
  GetEdges(QueryContext* qctx, PlanNode* input, GraphSpaceID space, Expression* src, Expression* type,
           Expression* ranking, Expression* dst, std::vector<storage::cpp2::EdgeProp> props,
           std::vector<storage::cpp2::Expr> exprs, bool dedup, int64_t limit,
           std::vector<storage::cpp2::OrderBy> orderBy, std::string filter)
      : Explore(qctx, Kind::kGetEdges, input, space, dedup, limit, std::move(filter), std::move(orderBy)),
        src_(src),
        type_(type),
        ranking_(ranking),
        dst_(dst),
...
class IndexScan final : public Explore {
 public:
  ...
  static IndexScan* make(QueryContext* qctx, PlanNode* input, GraphSpaceID space, IndexQueryCtx&& contexts,
                         IndexReturnCols&& returnCols, bool isEdge, int32_t schemaId, bool isEmptyResultSet = false,
                         bool dedup = false, std::vector<storage::cpp2::OrderBy> orderBy = {},
                         int64_t limit = std::numeric_limits<int64_t>::max(), std::string filter = "") {
    return qctx->objPool()->add(new IndexScan(qctx, input, space, std::move(contexts), std::move(returnCols), isEdge,
                                              schemaId, isEmptyResultSet, dedup, std::move(orderBy), limit,
                                              std::move(filter)));
  }
  ...
  const std::vector<storage::cpp2::IndexQueryContext>* queryContext() const { return contexts_.get(); }

  const std::vector<std::string>* returnColumns() const { return returnCols_.get(); }
  ...
  void setIndexQueryContext(IndexQueryCtx contexts) { contexts_ = std::move(contexts); }

  void setReturnCols(IndexReturnCols cols) { returnCols_ = std::move(cols); }
 ...
 private:
  IndexScan(QueryContext* qctx, PlanNode* input, GraphSpaceID space, IndexQueryCtx&& contexts,
            IndexReturnCols&& returnCols, bool isEdge, int32_t schemaId, bool isEmptyResultSet, bool dedup,
            std::vector<storage::cpp2::OrderBy> orderBy, int64_t limit, std::string filter)
      : Explore(qctx, Kind::kIndexScan, input, space, dedup, limit, std::move(filter), std::move(orderBy)) {
...
class Filter final : public SingleInputNode {
 public:
  static Filter* make(QueryContext* qctx, PlanNode* input, Expression* condition, bool needStableFilter = false) {
...
class SetOp : public BiInputNode {
 protected:
  SetOp(QueryContext* qctx, Kind kind, PlanNode* left, PlanNode* right) : BiInputNode(qctx, kind, left, right) {
...
class Union final : public SetOp {
 ...
 private:
  Union(QueryContext* qctx, PlanNode* left, PlanNode* right) : SetOp(qctx, Kind::kUnion, left, right) {}
};
...
class Intersect final : public SetOp {
 ...
 private:
  Intersect(QueryContext* qctx, PlanNode* left, PlanNode* right) : SetOp(qctx, Kind::kIntersect, left, right) {}
};
...
class Minus final : public SetOp {
 ...
 private:
  Minus(QueryContext* qctx, PlanNode* left, PlanNode* right) : SetOp(qctx, Kind::kMinus, left, right) {}
};
...
class Project final : public SingleInputNode {
 public:
  static Project* make(QueryContext* qctx, PlanNode* input, YieldColumns* cols) {
...
class Sort final : public SingleInputNode {
 public:
  static Sort* make(QueryContext* qctx, PlanNode* input,
 ...
 private:
  Sort(QueryContext* qctx, PlanNode* input, std::vector<std::pair<size_t, OrderFactor::OrderType>> factors)
...
class TopN final : public SingleInputNode {
 public:
  static TopN* make(QueryContext* qctx, PlanNode* input, std::vector<std::pair<size_t, OrderFactor::OrderType>> factors,
 ...
 private:
  TopN(QueryContext* qctx, PlanNode* input, std::vector<std::pair<size_t, OrderFactor::OrderType>> factors,
       int64_t offset, int64_t count)
...
class Aggregate final : public SingleInputNode {
 public:
  static Aggregate* make(QueryContext* qctx, PlanNode* input, std::vector<Expression*>&& groupKeys,
                         std::vector<Expression*>&& groupItems) {
    return qctx->objPool()->add(new Aggregate(qctx, input, std::move(groupKeys), std::move(groupItems)));
  }
 ...
 private:
  Aggregate(QueryContext* qctx, PlanNode* input, std::vector<Expression*>&& groupKeys,
            std::vector<Expression*>&& groupItems)
...
class Dedup final : public SingleInputNode {
 public:
  static Dedup* make(QueryContext* qctx, PlanNode* input) { return qctx->objPool()->add(new Dedup(qctx, input)); }

 private:
  Dedup(QueryContext* qctx, PlanNode* input) : SingleInputNode(qctx, Kind::kDedup, input) {}
};
...
class DataCollect final : public SingleDependencyNode {
 public:
  ...
  static DataCollect* make(QueryContext* qctx, PlanNode* input, CollectKind collectKind,
                           std::vector<std::string> vars) {
    return qctx->objPool()->add(new DataCollect(qctx, input, collectKind, std::move(vars)));
  }

  void setMToN(StepClause::MToN* mToN) { mToN_ = mToN; }

  void setDistinct(bool distinct) { distinct_ = distinct; }

  CollectKind collectKind() const { return collectKind_; }

  std::vector<std::string> vars() const {
    std::vector<std::string> vars(inputVars_.size());
    std::transform(inputVars_.begin(), inputVars_.end(), vars.begin(), [](auto& var) { return var->name; });
    return vars;
  }

  StepClause::MToN* mToN() const { return mToN_; }
 ...
 private:
  DataCollect(QueryContext* qctx, PlanNode* input, CollectKind collectKind, std::vector<std::string> vars)
...
class Join : public SingleDependencyNode {
 ...
 protected:
  Join(QueryContext* qctx, Kind kind, PlanNode* input, std::pair<std::string, int64_t> leftVar,
       std::pair<std::string, int64_t> rightVar, std::vector<Expression*> hashKeys, std::vector<Expression*> probeKeys);
...
class LeftJoin final : public Join {
 public:
  static LeftJoin* make(QueryContext* qctx, PlanNode* input, std::pair<std::string, int64_t> leftVar,
                        std::pair<std::string, int64_t> rightVar, std::vector<Expression*> hashKeys,
                        std::vector<Expression*> probeKeys) {
    return qctx->objPool()->add(
        new LeftJoin(qctx, input, std::move(leftVar), std::move(rightVar), std::move(hashKeys), std::move(probeKeys)));
  }

  std::unique_ptr<PlanNodeDescription> explain() const override;

 private:
  LeftJoin(QueryContext* qctx, PlanNode* input, std::pair<std::string, int64_t> leftVar,
           std::pair<std::string, int64_t> rightVar, std::vector<Expression*> hashKeys,
           std::vector<Expression*> probeKeys)
      : Join(qctx, Kind::kLeftJoin, input, std::move(leftVar), std::move(rightVar), std::move(hashKeys),
             std::move(probeKeys)) {}
};
...
class InnerJoin final : public Join {
 public:
  static InnerJoin* make(QueryContext* qctx, PlanNode* input, std::pair<std::string, int64_t> leftVar,
                         std::pair<std::string, int64_t> rightVar, std::vector<Expression*> hashKeys,
                         std::vector<Expression*> probeKeys) {
    return qctx->objPool()->add(
        new InnerJoin(qctx, input, std::move(leftVar), std::move(rightVar), std::move(hashKeys), std::move(probeKeys)));
  }

  std::unique_ptr<PlanNodeDescription> explain() const override;

 private:
  InnerJoin(QueryContext* qctx, PlanNode* input, std::pair<std::string, int64_t> leftVar,
            std::pair<std::string, int64_t> rightVar, std::vector<Expression*> hashKeys,
            std::vector<Expression*> probeKeys)
      : Join(qctx, Kind::kInnerJoin, input, std::move(leftVar), std::move(rightVar), std::move(hashKeys),
             std::move(probeKeys)) {}
};
...
class Assign final : public SingleInputNode {
 public:
  static Assign* make(QueryContext* qctx, PlanNode* input) { return qctx->objPool()->add(new Assign(qctx, input)); }

  const std::vector<std::pair<std::string, std::unique_ptr<Expression>>>& items() const { return items_; }
...
class UnionAllVersionVar final : public SingleInputNode {
 ...
 private:
  UnionAllVersionVar(QueryContext* qctx, PlanNode* input) : SingleInputNode(qctx, Kind::kUnionAllVersionVar, input) {}
};
