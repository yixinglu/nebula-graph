...
namespace nebula {
namespace graph {
PlanNode* InnerJoinStrategy::connect(const PlanNode* left, const PlanNode* right) { return joinDataSet(left, right); }
...
PlanNode* InnerJoinStrategy::joinDataSet(const PlanNode* left, const PlanNode* right) {
  ...
  auto join = InnerJoin::make(qctx_, const_cast<PlanNode*>(right), {left->outputVar(), 0}, {right->outputVar(), 0},
                              {buildExpr}, {probeExpr});
