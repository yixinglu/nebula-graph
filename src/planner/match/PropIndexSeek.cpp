...
bool PropIndexSeek::matchNode(NodeContext* nodeCtx) {
  ...
  if (matchClauseCtx->where != nullptr && matchClauseCtx->where->filter != nullptr) {
    filter = MatchSolver::makeIndexFilter(*node.labels.back(), *node.alias, matchClauseCtx->where->filter.get(),
                                          matchClauseCtx->qctx);
  }
  if (filter == nullptr) {
...
StatusOr<SubPlan> PropIndexSeek::transformNode(NodeContext* nodeCtx) {
  ...
  auto scan = IndexScan::make(matchClauseCtx->qctx, nullptr, matchClauseCtx->space.id, std::move(contexts),
                              std::move(columns), false, nodeCtx->scanInfo.schemaIds.back());
