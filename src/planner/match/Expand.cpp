...
static std::unique_ptr<std::vector<VertexProp>> genVertexProps() { return std::make_unique<std::vector<VertexProp>>(); }
...
Status Expand::expandSteps(const NodeInfo& node, const EdgeInfo& edge, SubPlan* plan) {
  ...
  if (minHop == 0) {
    ...
    startIndex = 0;
    // Get vertex
    NG_RETURN_IF_ERROR(MatchSolver::appendFetchVertexPlan(node.filter, matchCtx_->space, matchCtx_->qctx,
                                                          initialExpr_.release(), inputVar_, subplan));
...
Status Expand::expandStep(const EdgeInfo& edge, PlanNode* dep, const std::string& inputVar,
                          const Expression* nodeFilter, SubPlan* plan) {
...
Status Expand::collectData(const PlanNode* joinLeft, const PlanNode* joinRight, PlanNode** passThrough, SubPlan* plan) {
  auto qctx = matchCtx_->qctx;
...
Status Expand::filterDatasetByPathLength(const EdgeInfo& edge, PlanNode* input, SubPlan* plan) {
  ...
  auto expr =
      std::make_unique<RelationalExpression>(Expression::Kind::kRelGE, edgeExpr.release(), minHopExpr.release());
...
Expression* Expand::buildNStepLoopCondition(int64_t startIndex, int64_t maxHop) const {
  ...
  return matchCtx_->qctx->objPool()->add(new RelationalExpression(
      ...
      new UnaryExpression(Expression::Kind::kUnaryIncr, new VariableExpression(new std::string(loopSteps))),
