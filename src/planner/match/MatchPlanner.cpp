...
namespace nebula {
namespace graph {
bool MatchPlanner::match(AstContext* astCtx) { return astCtx->sentence->kind() == Sentence::Kind::kMatch; }
...
StatusOr<SubPlan> MatchPlanner::connectSegments(std::vector<SubPlan>& subplans,
  ...
  for (size_t i = 0; i < subplans.size() - 1; ++i) {
    auto interimPlan =
        SegmentsConnector::connectSegments(clauses[i + 1].get(), clauses[i].get(), subplans[i + 1], finalPlan);
