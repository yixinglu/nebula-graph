...
Status MatchClausePlanner::findStarts(MatchClauseContext* matchClauseCtx, bool& startFromEdge, size_t& startIndex,
                                      SubPlan& matchClausePlan) {
  auto& nodeInfos = matchClauseCtx->nodeInfos;
  auto& edgeInfos = matchClauseCtx->edgeInfos;
...
Status MatchClausePlanner::expand(const std::vector<NodeInfo>& nodeInfos, const std::vector<EdgeInfo>& edgeInfos,
                                  MatchClauseContext* matchClauseCtx, bool startFromEdge, size_t startIndex,
                                  SubPlan& subplan) {
  if (startFromEdge) {
...
Status MatchClausePlanner::expandFromNode(const std::vector<NodeInfo>& nodeInfos,
                                          const std::vector<EdgeInfo>& edgeInfos, MatchClauseContext* matchClauseCtx,
                                          size_t startIndex, SubPlan& subplan) {
  ...
  // Pattern: ()-[]-...-(start)-...-[]-()
  NG_RETURN_IF_ERROR(rightExpandFromNode(nodeInfos, edgeInfos, matchClauseCtx, startIndex, subplan));
  auto left = subplan.root;
  NG_RETURN_IF_ERROR(leftExpandFromNode(nodeInfos, edgeInfos, matchClauseCtx, startIndex, var, subplan));

  // Connect the left expand and right expand part.
  auto right = subplan.root;
  subplan.root = SegmentsConnector::innerJoinSegments(matchClauseCtx->qctx, left, right, JoinStrategyPos::kStart,
                                                      JoinStrategyPos::kStart);
  return Status::OK();
}
...
Status MatchClausePlanner::leftExpandFromNode(const std::vector<NodeInfo>& nodeInfos,
                                              ...
                                              std::string inputVar, SubPlan& subplan) {
  std::vector<std::string> joinColNames = {folly::stringPrintf("%s_%lu", kPathStr, nodeInfos.size())};
  for (size_t i = startIndex; i > 0; --i) {
    auto left = subplan.root;
    auto status = std::make_unique<Expand>(matchClauseCtx, i == startIndex ? initialExpr_->clone() : nullptr)
    ...
    if (i < startIndex) {
      auto right = subplan.root;
      VLOG(1) << "left: " << folly::join(",", left->colNames()) << " right: " << folly::join(",", right->colNames());
  ...
  auto left = subplan.root;
  NG_RETURN_IF_ERROR(
      MatchSolver::appendFetchVertexPlan(nodeInfos.front().filter, matchClauseCtx->space, matchClauseCtx->qctx,
                                         edgeInfos.empty() ? initialExpr_->clone().release() : nullptr, subplan));
  if (!edgeInfos.empty()) {
    auto right = subplan.root;
    VLOG(1) << "left: " << folly::join(",", left->colNames()) << " right: " << folly::join(",", right->colNames());
    subplan.root = SegmentsConnector::innerJoinSegments(matchClauseCtx->qctx, left, right);
    joinColNames.emplace_back(folly::stringPrintf("%s_%lu", kPathStr, nodeInfos.size() + startIndex));
    subplan.root->setColNames(joinColNames);
  }
...
Status MatchClausePlanner::rightExpandFromNode(const std::vector<NodeInfo>& nodeInfos,
  ...
  auto left = subplan.root;
  NG_RETURN_IF_ERROR(
      MatchSolver::appendFetchVertexPlan(nodeInfos.back().filter, matchClauseCtx->space, matchClauseCtx->qctx,
                                         edgeInfos.empty() ? initialExpr_->clone().release() : nullptr, subplan));
  if (!edgeInfos.empty()) {
    auto right = subplan.root;
    VLOG(1) << "left: " << folly::join(",", left->colNames()) << " right: " << folly::join(",", right->colNames());
...
Status MatchClausePlanner::projectColumnsBySymbols(MatchClauseContext* matchClauseCtx, size_t startIndex,
                                                   SubPlan& plan) {
  ...
  auto addNode = [&, this](size_t i) {
    ...
    if (nodeInfo.alias != nullptr && !nodeInfo.anonymous) {
      if (i >= startIndex) {
        ...
        columns->addColumn(buildVertexColumn(inColNames[nodeInfos.size() - i], *nodeInfo.alias));
      }
  ...
  auto iter =
      std::find_if(aliases.begin(), aliases.end(), [](const auto& alias) { return alias.second == AliasType::kPath; });
...
YieldColumn* MatchClausePlanner::buildVertexColumn(const std::string& colName, const std::string& alias) const {
...
YieldColumn* MatchClausePlanner::buildPathColumn(const std::string& alias, size_t startIndex,
                                                 const std::vector<std::string> colNames, size_t nodeInfoSize) const {
  ...
  if (leftExpandPath->size() != 0) {
    ...
    auto reversePath = std::make_unique<FunctionCallExpression>(new std::string("reversePath"), args);
