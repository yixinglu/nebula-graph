...
void YieldClausePlanner::rewriteYieldColumns(const YieldClauseContext* yctx, const YieldColumns* yields,
...
void YieldClausePlanner::rewriteGroupExprs(const YieldClauseContext* yctx, const std::vector<Expression*>* exprs,
...
Status YieldClausePlanner::buildYield(YieldClauseContext* yctx, SubPlan& subplan) {
  ...
  if (!yctx->hasAgg_) {
    ...
    auto* agg = Aggregate::make(yctx->qctx, currentRoot, std::move(newGroupKeys), std::move(newGroupItems));
