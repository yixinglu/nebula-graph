...
Expression* MatchSolver::rewriteLabel2VarProp(const Expression* expr) {
  ...
  auto rewriter = [](const Expression* e) -> Expression* {
    ...
    if (e->kind() == Expression::Kind::kLabelAttribute) {
      auto* la = static_cast<const LabelAttributeExpression*>(e);
      auto* var = new VariablePropertyExpression(new std::string(), new std::string(*la->left()->name()));
...
Expression* MatchSolver::doRewrite(const std::unordered_map<std::string, AliasType>& aliases, const Expression* expr) {
...
Expression* MatchSolver::makeIndexFilter(const std::string& label, const MapExpression* map, QueryContext* qctx) {
  auto& items = map->items();
  Expression* root = new RelationalExpression(
      Expression::Kind::kRelEQ, new TagPropertyExpression(new std::string(label), new std::string(*items[0].first)),
      items[0].second->clone().release());
  for (auto i = 1u; i < items.size(); i++) {
    auto* left = root;
    auto* right = new RelationalExpression(
        Expression::Kind::kRelEQ, new TagPropertyExpression(new std::string(label), new std::string(*items[i].first)),
...
Expression* MatchSolver::makeIndexFilter(const std::string& label, const std::string& alias, Expression* filter,
                                         QueryContext* qctx) {
  static const std::unordered_set<Expression::Kind> kinds = {
      Expression::Kind::kRelEQ, Expression::Kind::kRelLT, Expression::Kind::kRelLE,
      Expression::Kind::kRelGT, Expression::Kind::kRelGE,
  };
  ...
  for (auto* item : ands) {
    ...
    if (left->kind() == Expression::Kind::kLabelAttribute && right->kind() == Expression::Kind::kConstant) {
      la = static_cast<const LabelAttributeExpression*>(left);
      constant = static_cast<const ConstantExpression*>(right);
    } else if (right->kind() == Expression::Kind::kLabelAttribute && left->kind() == Expression::Kind::kConstant) {
    ...
    auto* tpExpr = new TagPropertyExpression(new std::string(label), new std::string(value.getStr()));
...
void MatchSolver::extractAndDedupVidColumn(QueryContext* qctx, Expression* initialExpr, PlanNode* dep,
                                           const std::string& inputVar, SubPlan& plan) {
...
Expression* MatchSolver::initialExprOrEdgeDstExpr(Expression* initialExpr, const std::string& vidCol) {
...
PlanNode* MatchSolver::filtPathHasSameEdge(PlanNode* input, const std::string& column, QueryContext* qctx) {
  auto args = std::make_unique<ArgumentList>();
  ...
  auto cond = std::make_unique<RelationalExpression>(Expression::Kind::kRelEQ, fnCall.release(), falseConst.release());
  auto filter = Filter::make(qctx, input, qctx->objPool()->add(cond.release()));
  filter->setColNames(input->colNames());
  return filter;
}

Status MatchSolver::appendFetchVertexPlan(const Expression* nodeFilter, const SpaceInfo& space, QueryContext* qctx,
                                          Expression* initialExpr, SubPlan& plan) {
  return appendFetchVertexPlan(nodeFilter, space, qctx, initialExpr, plan.root->outputVar(), plan);
}

Status MatchSolver::appendFetchVertexPlan(const Expression* nodeFilter, const SpaceInfo& space, QueryContext* qctx,
                                          Expression* initialExpr, std::string inputVar, SubPlan& plan) {
  // [Project && Dedup]
  ...
  auto gv = GetVertices::make(qctx, plan.root, space.id, qctx->objPool()->add(srcExpr.release()),
...
StatusOr<std::vector<storage::cpp2::VertexProp>> MatchSolver::flattenTags(QueryContext* qctx, const SpaceInfo& space) {
