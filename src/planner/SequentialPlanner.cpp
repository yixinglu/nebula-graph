...
namespace nebula {
namespace graph {
bool SequentialPlanner::match(AstContext* astCtx) { return astCtx->sentence->kind() == Sentence::Kind::kSequential; }
...
void SequentialPlanner::ifBuildDataCollect(SubPlan& subPlan, QueryContext* qctx) {
  switch (subPlan.root->kind()) {
    ...
    case PlanNode::Kind::kFilter: {
      auto* dc =
          DataCollect::make(qctx, subPlan.root, DataCollect::CollectKind::kRowBasedMove, {subPlan.root->outputVar()});
