...
class SingleDependencyNode : public PlanNode {
 ...
 protected:
  SingleDependencyNode(QueryContext* qctx, Kind kind, const PlanNode* dep) : PlanNode(qctx, kind) { addDep(dep); }

  void clone(const SingleDependencyNode& node) { PlanNode::clone(node); }
...
class SingleInputNode : public SingleDependencyNode {
 ...
 protected:
  void clone(const SingleInputNode& node) { SingleDependencyNode::clone(node); }

  SingleInputNode(QueryContext* qctx, Kind kind, const PlanNode* dep) : SingleDependencyNode(qctx, kind, dep) {
    if (dep != nullptr) {
