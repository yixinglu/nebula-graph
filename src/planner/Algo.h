...
class BFSShortestPath : public SingleInputNode {
 ...
 private:
  BFSShortestPath(QueryContext* qctx, PlanNode* input) : SingleInputNode(qctx, Kind::kBFSShortest, input) {}
};
...
class ConjunctPath : public BiInputNode {
 public:
  ...
  static ConjunctPath* make(QueryContext* qctx, PlanNode* left, PlanNode* right, PathKind pathKind, size_t steps) {
...
class ProduceAllPaths final : public SingleInputNode {
 ...
 private:
  ProduceAllPaths(QueryContext* qctx, PlanNode* input) : SingleInputNode(qctx, Kind::kProduceAllPaths, input) {}
...
class CartesianProduct final : public SingleDependencyNode {
 ...
 private:
  CartesianProduct(QueryContext* qctx, PlanNode* input) : SingleDependencyNode(qctx, Kind::kCartesianProduct, input) {}
...
class Subgraph final : public SingleInputNode {
 public:
  static Subgraph* make(QueryContext* qctx, PlanNode* input, const std::string& oneMoreStepOutput,
                        const std::string& currentStepVar, uint32_t steps) {
    return qctx->objPool()->add(new Subgraph(qctx, input, oneMoreStepOutput, currentStepVar, steps));
  }
 ...
 private:
  Subgraph(QueryContext* qctx, PlanNode* input, const std::string& oneMoreStepOutput, const std::string& currentStepVar,
           uint32_t steps)
