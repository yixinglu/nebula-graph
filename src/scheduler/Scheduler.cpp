...
folly::Future<Status> Scheduler::doSchedule(Executor *executor) {
  switch (executor->node()->kind()) {
    ...
    case PlanNode::Kind::kLoop: {
      auto loop = static_cast<LoopExecutor *>(executor);
      return doScheduleParallel(loop->depends()).thenValue(task(loop, [loop, this](Status status) {
    ...
    case PlanNode::Kind::kPassThrough: {
      ...
      return doScheduleParallel(mout->depends()).thenValue(task(mout, [&data, mout, this](Status status) {
    ...
    default: {
      ...
      return doScheduleParallel(deps).thenValue(task(executor, [executor, this](Status stats) {
