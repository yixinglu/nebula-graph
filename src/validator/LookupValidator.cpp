...
Status LookupValidator::toPlan() {
  auto* is = IndexScan::make(qctx_, nullptr, spaceId_, std::move(contexts_), std::move(returnCols_), isEdge_, schemaId_,
                             isEmptyResultSet_);
...
Status LookupValidator::prepareYield() {
  ...
  if (isEdge_) {
    ...
    newYieldColumns_->addColumn(
        new YieldColumn(new InputPropertyExpression(new std::string(kSrcVID)), new std::string(kSrcVID)));
    newYieldColumns_->addColumn(
        new YieldColumn(new InputPropertyExpression(new std::string(kDstVID)), new std::string(kDstVID)));
    newYieldColumns_->addColumn(
        new YieldColumn(new InputPropertyExpression(new std::string(kRanking)), new std::string(kRanking)));
  } else {
    newYieldColumns_->addColumn(
        new YieldColumn(new InputPropertyExpression(new std::string(kVertexID)), new std::string(kVertexID)));
  }
  ...
  for (auto col : columns) {
    ...
    if (col->expr()->kind() == Expression::Kind::kLabelAttribute) {
      ...
      if (isEdge_) {
        newYieldColumns_->addColumn(
            new YieldColumn(new EdgePropertyExpression(new std::string(schemaName), new std::string(colName))));
      } else {
        newYieldColumns_->addColumn(
            new YieldColumn(new TagPropertyExpression(new std::string(schemaName), new std::string(colName))));
      }
      ...
      if (ret == meta::cpp2::PropertyType::UNKNOWN) {
        return Status::SemanticError("Column %s not found in schema %s", colName.c_str(), from_.c_str());
      }
      ...
      return Status::SemanticError("Yield clauses are not supported : %s", col->expr()->toString().c_str());
    }
  }
  return Status::OK();
}
  ...
  for (const auto& row : vRet.value()) {
    ...
    if (isEdge_) {
      r = std::make_unique<RelationalExpression>(
          ...
          new EdgePropertyExpression(new std::string(*tsExpr->arg()->from()), new std::string(*tsExpr->arg()->prop())),
          new ConstantExpression(Value(row)));
    } else {
      r = std::make_unique<RelationalExpression>(
          Expression::Kind::kRelEQ,
          new TagPropertyExpression(new std::string(*tsExpr->arg()->from()), new std::string(*tsExpr->arg()->prop())),
          new ConstantExpression(Value(row)));
    }
  ...
  while (--retryCnt > 0) {
    ...
    switch (expr->kind()) {
      case Expression::Kind::kTSFuzzy: {
        ...
        ret = nebula::plugin::ESGraphAdapter::kAdapter->fuzzy(randomFTClient(), doc, limit, fuzz, op, result);
        break;
      }
      case Expression::Kind::kTSPrefix: {
        ret = nebula::plugin::ESGraphAdapter::kAdapter->prefix(randomFTClient(), doc, limit, result);
        break;
      }
      case Expression::Kind::kTSRegexp: {
        ret = nebula::plugin::ESGraphAdapter::kAdapter->regexp(randomFTClient(), doc, limit, result);
        break;
      }
      case Expression::Kind::kTSWildcard: {
        ret = nebula::plugin::ESGraphAdapter::kAdapter->wildcard(randomFTClient(), doc, limit, result);
        break;
      }
      default:
...
Status LookupValidator::checkRelExpr(RelationalExpression* expr) {
  ...
  if (left->kind() == Expression::Kind::kLabelAttribute && right->kind() == Expression::Kind::kLabelAttribute) {
    return Status::SemanticError("Expression %s not supported yet", expr->toString().c_str());
  } else if (left->kind() == Expression::Kind::kLabelAttribute || right->kind() == Expression::Kind::kLabelAttribute) {
...
Status LookupValidator::rewriteRelExpr(RelationalExpression* expr) {
  ...
  auto* la = leftIsAE ? static_cast<LabelAttributeExpression*>(left) : static_cast<LabelAttributeExpression*>(right);
  ...
  auto c =
      leftIsAE ? checkConstExpr(right, prop, relExprType, leftIsAE) : checkConstExpr(left, prop, relExprType, leftIsAE);
...
StatusOr<Value> LookupValidator::checkConstExpr(Expression* expr, const std::string& prop, const Expression::Kind kind,
                                                bool leftIsAE) {
