...
class MatchValidator final : public TraversalValidator {
 ...
 private:
  ...
  Status validateReturn(MatchReturn *ret, const CypherClauseContextBase *cypherClauseCtx,
  ...
  Status validatePagination(const Expression *skipExpr, const Expression *limitExpr,
                            PaginationContext &paginationCtx) const;

  Status validateOrderBy(const OrderFactors *factors, const YieldColumns *yieldColumns,
  ...
  StatusOr<Expression *> makeSubFilter(const std::string &alias, const MapExpression *map,
                                       const std::string &label = "") const;
  StatusOr<Expression *> makeSubFilterWithoutSave(const std::string &alias, const MapExpression *map,
                                                  const std::string &label = "") const;
  ...
  Status buildNodeInfo(const MatchPath *path, std::vector<NodeInfo> &edgeInfos,
                       std::unordered_map<std::string, AliasType> &aliases) const;

  Status buildEdgeInfo(const MatchPath *path, std::vector<EdgeInfo> &nodeInfos,
                       std::unordered_map<std::string, AliasType> &aliases) const;
