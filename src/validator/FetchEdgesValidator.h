...
class FetchEdgesValidator final : public Validator {
 ...
 private:
  ...
  Expression* notEmpty(Expression* expr) {
    return new RelationalExpression(Expression::Kind::kRelNE, new ConstantExpression(Value::kEmpty),
                                    DCHECK_NOTNULL(expr));
  }

  Expression* lgAnd(Expression* left, Expression* right) {
    return new LogicalExpression(Expression::Kind::kLogicalAnd, DCHECK_NOTNULL(left), DCHECK_NOTNULL(right));
  }
