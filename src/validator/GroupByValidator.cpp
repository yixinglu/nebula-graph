...
Status GroupByValidator::validateYield(const YieldClause* yieldClause) {
  ...
  for (auto* col : columns) {
    ...
    if (!rewrited) {
      ...
      projCols_->addColumn(new YieldColumn(new VariablePropertyExpression(new std::string(), new std::string(name)),
...
Status GroupByValidator::validateGroup(const GroupClause* groupClause) {
  ...
  auto groupByValid = [](Expression::Kind kind) -> bool {
    return std::unordered_set<Expression::Kind>{Expression::Kind::kAdd,          Expression::Kind::kMinus,
                                                Expression::Kind::kMultiply,     Expression::Kind::kDivision,
                                                Expression::Kind::kMod,          Expression::Kind::kTypeCasting,
                                                Expression::Kind::kFunctionCall, Expression::Kind::kInputProperty,
                                                Expression::Kind::kVarProperty,  Expression::Kind::kCase}
        .count(kind);
  };
  for (auto* col : columns) {
...
Status GroupByValidator::rewriteInnerAggExpr(YieldColumn* col, bool& rewrited) {
  ...
  if (aggs.size() > 1) {
    return Status::SemanticError("Aggregate function nesting is not allowed: `%s'", collectAggCol->toString().c_str());
  }
  if (aggs.size() == 1) {
