...
StatusOr<GetNeighbors::EdgeProps> GetSubgraphValidator::buildEdgeProps() {
  if (edgeTypes_.empty()) {
    ...
    return std::make_unique<std::vector<storage::cpp2::EdgeProp>>(std::move(allEdgePropResult).value());
  }
  auto edgePropResult = fillEdgeProp(edgeTypes_);
  NG_RETURN_IF_ERROR(edgePropResult);
  return std::make_unique<std::vector<storage::cpp2::EdgeProp>>(std::move(edgePropResult).value());
}
...
Status GetSubgraphValidator::zeroStep(PlanNode* depend, const std::string& inputVar) {
  ...
  auto* getVertex = GetVertices::make(qctx_, depend, space.id, from_.src, std::move(vertexPropsResult).value(),
                                      std::move(exprs), true);
...
Status GetSubgraphValidator::toPlan() {
  ...
  gn->setSrc(from_.src);
  gn->setVertexProps(std::make_unique<std::vector<storage::cpp2::VertexProp>>(std::move(vertexPropsResult).value()));
  auto edgePropsResult = buildEdgeProps();
  NG_RETURN_IF_ERROR(edgePropsResult);
  gn->setEdgeProps(std::make_unique<std::vector<storage::cpp2::EdgeProp>>(*edgePropsResult.value()));
  ...
  auto* dc = DataCollect::make(qctx_, loop, DataCollect::CollectKind::kSubgraph, std::move(collects));
