...
Status CreateTagValidator::validateImpl() {
  ...
  if (pro != nullptr) {
    return Status::SemanticError("Has the same name `%s' in the SequentialSentences", name_.c_str());
  }
...
Status CreateTagValidator::toPlan() {
  ...
  auto doNode = CreateTag::make(qctx_, plan->root(), std::move(name_), std::move(schema_), ifNotExist_);
  root_ = doNode;
  tail_ = root_;
  return Status::OK();
}
...
Status CreateEdgeValidator::toPlan() {
  auto *plan = qctx_->plan();
  auto doNode = CreateEdge::make(qctx_, plan->root(), std::move(name_), std::move(schema_), ifNotExist_);
  root_ = doNode;
  tail_ = root_;
  return Status::OK();
}
...
Status AlterTagValidator::toPlan() {
  auto *doNode = AlterTag::make(qctx_, nullptr, vctx_->whichSpace().id, std::move(name_), std::move(schemaItems_),
...
Status AlterEdgeValidator::toPlan() {
  auto *doNode = AlterEdge::make(qctx_, nullptr, vctx_->whichSpace().id, std::move(name_), std::move(schemaItems_),
                                 std::move(schemaProp_));
  root_ = doNode;
  tail_ = root_;
  return Status::OK();
...
Status CreateTagIndexValidator::toPlan() {
  auto sentence = static_cast<CreateTagIndexSentence *>(sentence_);
  auto *doNode = CreateTagIndex::make(qctx_, nullptr, *sentence->tagName(), *sentence->indexName(), sentence->fields(),
...
Status CreateEdgeIndexValidator::toPlan() {
  auto sentence = static_cast<CreateEdgeIndexSentence *>(sentence_);
  auto *doNode = CreateEdgeIndex::make(qctx_, nullptr, *sentence->edgeName(), *sentence->indexName(),
                                       sentence->fields(), sentence->isIfNotExist());
  root_ = doNode;
  tail_ = root_;
  return Status::OK();
}
...
Status ShowTagIndexesValidator::validateImpl() { return Status::OK(); }
...
Status ShowEdgeIndexesValidator::validateImpl() { return Status::OK(); }
...
Status AddGroupValidator::validateImpl() { return Status::OK(); }

Status AddGroupValidator::toPlan() {
  auto sentence = static_cast<AddGroupSentence *>(sentence_);
  auto *doNode = AddGroup::make(qctx_, nullptr, *sentence->groupName(), sentence->zoneNames()->zoneNames());
  root_ = doNode;
  tail_ = root_;
  return Status::OK();
}
...
Status AddZoneIntoGroupValidator::toPlan() {
  ...
  auto *doNode = AddZoneIntoGroup::make(qctx_, nullptr, *sentence->groupName(), *sentence->zoneName());
  root_ = doNode;
  tail_ = root_;
  return Status::OK();
}
...
Status DropZoneFromGroupValidator::toPlan() {
  ...
  auto *doNode = DropZoneFromGroup::make(qctx_, nullptr, *sentence->groupName(), *sentence->zoneName());
  root_ = doNode;
  tail_ = root_;
  return Status::OK();
}
...
Status DropHostFromZoneValidator::toPlan() {
  ...
  auto *doNode = DropHostFromZone::make(qctx_, nullptr, *sentence->zoneName(), *sentence->address());
  root_ = doNode;
  tail_ = root_;
  return Status::OK();
}
