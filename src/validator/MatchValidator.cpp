...
namespace nebula {
namespace graph {
MatchValidator::MatchValidator(Sentence *sentence, QueryContext *context) : TraversalValidator(sentence, context) {
...
Status MatchValidator::validateImpl() {
  ...
  for (size_t i = 0; i < clauses.size(); ++i) {
    ...
    if (i > 0 && kind == ReadingClause::Kind::kMatch) {
      return Status::SemanticError("Match clause is not supported to be followed by other cypher clauses");
    }
    switch (kind) {
      ...
      case ReadingClause::Kind::kUnwind: {
        ...
        if (aliasesUsed) {
          NG_RETURN_IF_ERROR(combineAliases(unwindClauseCtx->aliasesGenerated, *aliasesUsed));
        }
        aliasesUsed = &unwindClauseCtx->aliasesGenerated;
        if (prevYieldColumns) {
          NG_RETURN_IF_ERROR(
              combineYieldColumns(const_cast<YieldColumns *>(unwindClauseCtx->yieldColumns), prevYieldColumns));
        }
...
Status MatchValidator::validatePath(const MatchPath *path, MatchClauseContext &matchClauseCtx) const {
  NG_RETURN_IF_ERROR(buildNodeInfo(path, matchClauseCtx.nodeInfos, matchClauseCtx.aliasesGenerated));
  NG_RETURN_IF_ERROR(buildEdgeInfo(path, matchClauseCtx.edgeInfos, matchClauseCtx.aliasesGenerated));
  NG_RETURN_IF_ERROR(buildPathExpr(path, matchClauseCtx));
  return Status::OK();
}

Status MatchValidator::buildPathExpr(const MatchPath *path, MatchClauseContext &matchClauseCtx) const {
  ...
  for (size_t i = 0; i < edgeInfos.size(); ++i) {
    pathBuild->add(
        std::make_unique<VariablePropertyExpression>(new std::string(), new std::string(*nodeInfos[i].alias)));
    pathBuild->add(
        std::make_unique<VariablePropertyExpression>(new std::string(), new std::string(*edgeInfos[i].alias)));
  }
  pathBuild->add(
      std::make_unique<VariablePropertyExpression>(new std::string(), new std::string(*nodeInfos.back().alias)));
  matchClauseCtx.pathBuild = std::move(pathBuild);
  return Status::OK();
}

Status MatchValidator::buildNodeInfo(const MatchPath *path, std::vector<NodeInfo> &nodeInfos,
...
Status MatchValidator::buildEdgeInfo(const MatchPath *path, std::vector<EdgeInfo> &edgeInfos,
                                     std::unordered_map<std::string, AliasType> &aliases) const {
  auto *sm = qctx_->schemaMng();
  auto steps = path->steps();
...
Status MatchValidator::validateFilter(const Expression *filter, WhereClauseContext &whereClauseCtx) const {
...
Status MatchValidator::validateReturn(MatchReturn *ret, const CypherClauseContextBase *cypherClauseCtx,
                                      ReturnClauseContext &retClauseCtx) const {
  auto kind = cypherClauseCtx->kind;
  if (kind != CypherClauseKind::kMatch && kind != CypherClauseKind::kUnwind && kind != CypherClauseKind::kWith) {
  ...
  for (auto *col : retClauseCtx.yield->yieldColumns->columns()) {
    if (!retClauseCtx.yield->hasAgg_ && ExpressionUtils::hasAny(col->expr(), {Expression::Kind::kAggregate})) {
...
Status MatchValidator::validateStepRange(const MatchStepRange *range) const {
  ...
  auto max = range->max();
  if (min > max) {
    return Status::SemanticError("Max hop must be greater equal than min hop: %ld vs. %ld", max, min);
  }
  ...
  if (min < 0) {
    return Status::SemanticError("Cannot set negtive steps minumum hop for variable length relationships");
  }
  return Status::OK();
}

Status MatchValidator::validateWith(const WithClause *with, WithClauseContext &withClauseCtx) const {
  // Check all referencing expressions are valid
  std::vector<const Expression *> exprs;
  ...
  for (auto *col : with->columns()->columns()) {
    ...
    if (!withClauseCtx.yield->hasAgg_ && ExpressionUtils::hasAny(col->expr(), {Expression::Kind::kAggregate})) {
...
Status MatchValidator::validateUnwind(const UnwindClause *unwind, UnwindClauseContext &unwindClauseCtx) const {
  ...
  NG_RETURN_IF_ERROR(validateAliases(std::vector<const Expression *>{expr}, unwindClauseCtx.aliasesUsed));
...
StatusOr<Expression *> MatchValidator::makeSubFilter(const std::string &alias, const MapExpression *map,
...
StatusOr<Expression *> MatchValidator::makeSubFilterWithoutSave(const std::string &alias, const MapExpression *map,
                                                                const std::string &label) const {
  ...
  if (items[0].second->kind() != Expression::Kind::kConstant) {
    return Status::SemanticError("Props must be constant: `%s'", items[0].second->toString().c_str());
  }
  Expression *root = new RelationalExpression(
      Expression::Kind::kRelEQ,
      new LabelAttributeExpression(new LabelExpression(alias), new ConstantExpression(*items[0].first)),
      items[0].second->clone().release());
  for (auto i = 1u; i < items.size(); i++) {
    if (items[i].second->kind() != Expression::Kind::kConstant) {
      return Status::SemanticError("Props must be constant: `%s'", items[i].second->toString().c_str());
    }
    auto *left = root;
    auto *right = new RelationalExpression(
        Expression::Kind::kRelEQ,
        new LabelAttributeExpression(new LabelExpression(alias), new ConstantExpression(*items[i].first)),
...
Status MatchValidator::combineYieldColumns(YieldColumns *yieldColumns, YieldColumns *prevYieldColumns) const {
  const auto &prevColumns = prevYieldColumns->columns();
  for (auto &column : prevColumns) {
    DCHECK(column->alias() != nullptr);
    auto *newColumn =
        new YieldColumn(new VariablePropertyExpression(new std::string(), new std::string(*column->alias())),
...
Status MatchValidator::validatePagination(const Expression *skipExpr, const Expression *limitExpr,
...
Status MatchValidator::validateOrderBy(const OrderFactors *factors, const YieldColumns *yieldColumns,
  ...
  for (auto *col : cols) {
    ...
    if (col->expr()->kind() != Expression::Kind::kAggregate) {
      ...
      if (size == 1) {
        ...
        yieldCtx.projCols_->addColumn(new YieldColumn(rewritedExpr, new std::string(colOldName)));
    ...
    if (!rewrited) {
      ...
      yieldCtx.projCols_->addColumn(new YieldColumn(
          new VariablePropertyExpression(new std::string(), new std::string(colNewName)), new std::string(colOldName)));
    } else {
