...
Status YieldValidator::validateImpl() {
  ...
  if (!groupByValidator_ && exprProps_.inputProps().empty() && exprProps_.varProps().empty() && inputVarName_.empty()) {
...
void YieldValidator::genConstantExprValues() {
  ...
  qctx_->ectx()->setResult(constantExprVar_, ResultBuilder().value(Value(std::move(ds))).finish());
}

Status YieldValidator::makeImplicitGroupByValidator() {
  auto *groupSentence = qctx()->objPool()->add(
      new GroupBySentence(static_cast<YieldSentence *>(sentence_)->yield()->clone().release(), nullptr, nullptr));
    ...
    if (expr->kind() == Expression::Kind::kInputProperty) {
      ...
      if (*vpe->prop() == "*") {
        ...
        for (auto &colDef : varColDefs) {
          auto newExpr = new VariablePropertyExpression(new std::string(*var), new std::string(colDef.name));
          NG_RETURN_IF_ERROR(makeOutputColumn(new YieldColumn(newExpr)));
        }
        continue;
      }
...
Status YieldValidator::toPlan() {
  ...
  if (yield->where()) {
    ...
    std::transform(inputs_.cbegin(), inputs_.cend(), colNames.begin(), [](auto &col) { return col.name; });
