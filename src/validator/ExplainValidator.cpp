...
ExplainValidator::ExplainValidator(Sentence* sentence, QueryContext* context) : Validator(sentence, context) {
...
static StatusOr<std::string> toExplainFormatType(const std::string& formatType) {
  ...
  std::transform(formatType.cbegin(), formatType.cend(), fmtType.begin(), [](char c) { return std::tolower(c); });
  ...
  return Status::SyntaxError("Invalid explain/profile format type: \"%s\", only following values are supported: %s",
                             fmtType.c_str(), allowedStr.c_str());
}
