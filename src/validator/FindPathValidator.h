...
class FindPathValidator final : public TraversalValidator {
 public:
  FindPathValidator(Sentence* sentence, QueryContext* context) : TraversalValidator(sentence, context) {}
 ...
 private:
  ...
  PlanNode* multiPairShortestPath(PlanNode* dep, Starts& starts, std::string& startVidsVar, std::string& pathVar,
                                  bool reverse);
  Expression* buildMultiPairLoopCondition(uint32_t steps, std::string conditionalVar);
  PlanNode* buildMultiPairFirstDataSet(PlanNode* dep, const std::string& inputVar, const std::string& outputVar);
