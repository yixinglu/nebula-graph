...
Status AdminJobValidator::validateImpl() {
  if (sentence_->getOp() == meta::cpp2::AdminJobOp::ADD) {
    ...
    if (requireSpace()) {
      ...
      if (cmd == meta::cpp2::AdminCmd::REBUILD_TAG_INDEX || cmd == meta::cpp2::AdminCmd::REBUILD_EDGE_INDEX) {
        auto ret = cmd == meta::cpp2::AdminCmd::REBUILD_TAG_INDEX ? qctx()->indexMng()->getTagIndexes(spaceId)
                                                                  : qctx()->indexMng()->getEdgeIndexes(spaceId);
        if (!ret.ok()) {
          return Status::SemanticError("Get index failed in space `%s': %s", spaceName.c_str(),
        ...
        for (auto i = 0u; i < paras.size() - 1; i++) {
          ...
          auto it =
              std::find_if(indexes.begin(), indexes.end(), [&indexName](std::shared_ptr<meta::cpp2::IndexItem> &item) {
                return item->get_index_name() == indexName;
              });
          if (it == indexes.end()) {
            return Status::SemanticError("Index %s not found in space %s", indexName.c_str(), spaceName.c_str());
          }
        }
      }
    }
...
Status AdminJobValidator::toPlan() {
  auto *doNode = SubmitJob::make(qctx_, nullptr, sentence_->getOp(), sentence_->getCmd(), sentence_->getParas());
  root_ = doNode;
  tail_ = root_;
  return Status::OK();
}
