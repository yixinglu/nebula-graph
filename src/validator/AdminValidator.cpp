...
Status CreateSpaceValidator::validateImpl() {
  ...
  for (auto &item : sentence->getOpts()) {
    switch (item->getOptType()) {
      ...
      case SpaceOptItem::VID_TYPE: {
        ...
        if (typeDef.type != meta::cpp2::PropertyType::INT64 && typeDef.type != meta::cpp2::PropertyType::FIXED_STRING) {
          std::stringstream ss;
        ...
        if (typeDef.type == meta::cpp2::PropertyType::INT64) {
          ...
          if (*typeDef.type_length_ref() <= 0) {
            return Status::SemanticError("Vid size should be a positive number: %d", *typeDef.type_length_ref());
          }
  ...
  if (!(*spaceDesc_.charset_name_ref()).empty() && !(*spaceDesc_.collate_name_ref()).empty()) {
    NG_RETURN_IF_ERROR(
        charsetInfo->charsetAndCollateMatch(*spaceDesc_.charset_name_ref(), *spaceDesc_.collate_name_ref()));
...
Status AddListenerValidator::validateImpl() {
  ...
  for (auto &host : hosts) {
    ...
    if (iter != storageHosts.end()) {
      return Status::Error("The listener host:%s couldn't on same with storage host info", host.toString().c_str());
    }
  }
  return Status::OK();
}
