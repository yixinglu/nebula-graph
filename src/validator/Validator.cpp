...
Validator::Validator(Sentence* sentence, QueryContext* qctx)
    : sentence_(DCHECK_NOTNULL(sentence)), qctx_(DCHECK_NOTNULL(qctx)), vctx_(DCHECK_NOTNULL(qctx->vctx())) {}
...
Status Validator::appendPlan(PlanNode* node, PlanNode* appended) {
  ...
  if (!node->isSingleInput()) {
    return Status::SemanticError("%s not support to append an input.", PlanNode::toString(node->kind()));
  }
...
Status Validator::validate() {
  ...
  if (!noSpaceRequired_) {
    ...
    VLOG(1) << "Space chosen, name: " << space_.spaceDesc.space_name_ref().value() << " id: " << space_.id;
  }
...
StatusOr<std::string> Validator::checkRef(const Expression* ref, Value::Type type) {
  ...
  if (ref->kind() == Expression::Kind::kVarProperty) {
    ...
    if (find == var.end()) {
      return Status::SemanticError("No property `%s' in variable `%s'", propExpr->prop()->c_str(), outputVar.c_str());
    }
