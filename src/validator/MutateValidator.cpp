...
Status InsertVerticesValidator::toPlan() {
  auto doNode =
      InsertVertices::make(qctx_, nullptr, spaceId_, std::move(vertices_), std::move(tagPropNames_), overwritable_);
  root_ = doNode;
  tail_ = root_;
  return Status::OK();
}
...
Status InsertEdgesValidator::toPlan() {
  ...
  auto doNode = InsertEdges::make(qctx_, nullptr, spaceId_, std::move(edges_), std::move(propNames_), overwritable_,
                                  useChainInsert);
  root_ = doNode;
  tail_ = root_;
  return Status::OK();
...
Status InsertEdgesValidator::prepareEdges() {
  ...
  for (auto i = 0u; i < rows_.size(); i++) {
    ...
    if (!evaluableExpr(row->srcid())) {
      ...
      return Status::SemanticError("Wrong src vid expression `%s'", row->srcid()->toString().c_str());
    }

    if (!evaluableExpr(row->dstid())) {
      LOG(ERROR) << "Wrong dst vid expression `" << row->dstid()->toString() << "\"";
      return Status::SemanticError("Wrong dst vid expression `%s'", row->dstid()->toString().c_str());
    }
...
Status DeleteVerticesValidator::validateImpl() {
  ...
  if (sentence->vertices()->isRef()) {
    ...
    if (type.value() != vidType_) {
      std::stringstream ss;
      ss << "The vid `" << vidRef_->toString() << "' should be type of `" << vidType_ << "', but was`" << type.value()
         << "'";
...
std::string DeleteVerticesValidator::buildVIds() {
  ...
  auto *vIds = new VariablePropertyExpression(new std::string(input), new std::string(kVid));
...
Status DeleteVerticesValidator::toPlan() {
  ...
  for (auto &name : edgeNames_) {
    auto *edgeKeyRef =
        new EdgeKeyRef(new EdgeSrcIdExpression(new std::string(name)), new EdgeDstIdExpression(new std::string(name)),
  ...
  auto *getNeighbors =
      GetNeighbors::make(qctx_, nullptr, spaceId_, vidRef_, edgeTypes_, storage::cpp2::EdgeDirection::BOTH, nullptr,
                         std::move(edgePropsPtr), std::move(statPropsPtr), std::move(exprPtr));
  getNeighbors->setInputVar(vidVar);

  // create deleteEdges node
  auto *deNode = DeleteEdges::make(qctx_, getNeighbors, spaceId_, std::move(edgeKeyRefs_));
...
Status DeleteEdgesValidator::buildEdgeKeyRef(const std::vector<EdgeKey *> &edgeKeys, const EdgeType edgeType) {
...
Status DeleteEdgesValidator::checkInput() {
  ...
  if (!exprProps_.srcTagProps().empty() || !exprProps_.dstTagProps().empty() || !exprProps_.edgeProps().empty()) {
...
Status DeleteEdgesValidator::toPlan() {
  auto *doNode = DeleteEdges::make(qctx_, nullptr, vctx_->whichSpace().id, edgeKeyRefs_);
...
Status UpdateValidator::checkAndResetSymExpr(Expression *inExpr, const std::string &symName, std::string &encodeStr) {
  bool hasWrongType = false;
...
std::unique_ptr<Expression> UpdateValidator::rewriteSymExpr(Expression *expr, const std::string &sym,
                                                            bool &hasWrongType, bool isEdge) {
...
Status UpdateVertexValidator::toPlan() {
  auto *update = UpdateVertex::make(qctx_, nullptr, spaceId_, std::move(name_), vId_, tagId_, insertable_,
                                    std::move(updatedProps_), std::move(returnProps_), std::move(condition_),
...
Status UpdateEdgeValidator::toPlan() {
  auto *outNode = UpdateEdge::make(qctx_, nullptr, spaceId_, name_, srcId_, dstId_, edgeType_, rank_, insertable_,
                                   updatedProps_, {}, condition_, {});
  using IsoLevel = meta::cpp2::IsolationLevel;
  auto isoLevel = space_.spaceDesc.isolation_level_ref().value_or(IsoLevel::DEFAULT);
  auto useToss = isoLevel == IsoLevel::TOSS;
  if (useToss) {
    ...
    auto *inNode = UpdateEdge::make(qctx_, outNode, spaceId_, std::move(name_), std::move(dstId_), std::move(srcId_),
                                    -edgeType_, rank_, insertable_, std::move(updatedProps_), std::move(returnProps_),
                                    std::move(condition_), std::move(yieldColNames_));
    root_ = inNode;
    tail_ = outNode;
  }
  return Status::OK();
