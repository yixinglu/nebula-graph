...
TEST_F(YieldValidatorTest, TypeCastTest) {
  ...
  }
  {
    ...
    auto result = checkResult(query);
    EXPECT_EQ(std::string(result.message()), "SemanticError: `(INT)\"123abc\"' is not a valid expression ");
  }
  {
    std::string query = "YIELD (int)\"abc123\"";
    auto result = checkResult(query);
    EXPECT_EQ(std::string(result.message()), "SemanticError: `(INT)\"abc123\"' is not a valid expression ");
  }
  {
  ...
  }
  {
    ...
    auto result = checkResult(query);
    EXPECT_EQ(std::string(result.message()), "SemanticError: `(FLOAT)\".a123\"' is not a valid expression ");
  }
  {
...
TEST_F(YieldValidatorTest, YieldPipe) {
  ...
  }
  {
    ...
    expected_ = {
        ...
        PlanNode::Kind::kGetNeighbors, PlanNode::Kind::kStart,
    };
    EXPECT_TRUE(checkResult(query, expected_));
  }
  {
...
TEST_F(YieldValidatorTest, Error) {
  ...
  }
  {
    ...
    auto result = checkResult(query);
    EXPECT_EQ(std::string(result.message()), "SemanticError: Only support input and variable in yield sentence.");
  }
  {
    auto query = var + "YIELD $^.person.name";
    auto result = checkResult(query);
...
TEST_F(YieldValidatorTest, AggCall) {
  ...
  }
  {
    ...
    expected_ = {
        ...
        PlanNode::Kind::kProject,     PlanNode::Kind::kGetNeighbors, PlanNode::Kind::kStart,
    };
    EXPECT_TRUE(checkResult(query, expected_));
  }
  {
