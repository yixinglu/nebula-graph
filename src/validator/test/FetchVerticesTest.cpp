...
TEST_F(FetchVerticesValidatorTest, FetchVerticesProp) {
  auto src = std::make_unique<VariablePropertyExpression>(new std::string("_VARNAME_"), new std::string("VertexID"));
  {
    ...
    prop.set_tag(tagId);
    auto *gv =
        GetVertices::make(qctx, start, 1, src.get(), std::vector<storage::cpp2::VertexProp>{std::move(prop)}, {});
    gv->setColNames({nebula::kVid, "person.name", "person.age"});
    // project
    auto yieldColumns = std::make_unique<YieldColumns>();
    yieldColumns->addColumn(new YieldColumn(new VertexExpression(), new std::string("vertices_")));
  ...
  // multi-tags
  {
    ...
    auto *gv =
        GetVertices::make(qctx, start, 1, src.get(),
  ...
  // With YIELD
  {
    ...
    expr1.set_expr(TagPropertyExpression(new std::string("person"), new std::string("name")).encode());
    storage::cpp2::Expr expr2;
    expr2.set_expr(TagPropertyExpression(new std::string("person"), new std::string("age")).encode());
    auto *gv = GetVertices::make(qctx, start, 1, src.get(), std::vector<storage::cpp2::VertexProp>{std::move(prop)},
    ...
    yieldColumns->addColumn(
        new YieldColumn(new InputPropertyExpression(new std::string(nebula::kVid)), new std::string("VertexID")));
    yieldColumns->addColumn(
        new YieldColumn(new TagPropertyExpression(new std::string("person"), new std::string("name"))));
    yieldColumns->addColumn(
        new YieldColumn(new TagPropertyExpression(new std::string("person"), new std::string("age"))));
    auto *project = Project::make(qctx, gv, yieldColumns.get());
  ...
  {
    auto qctx = getQCtx("FETCH PROP ON person,book \"1\" YIELD person.name, person.age, book.name");
    ...
    expr3.set_expr(TagPropertyExpression(new std::string("book"), new std::string("name")).encode());
    auto *gv = GetVertices::make(
        qctx, start, 1, src.get(), std::vector<storage::cpp2::VertexProp>{std::move(personProp), std::move(bookProp)},
    ...
    yieldColumns->addColumn(
        new YieldColumn(new TagPropertyExpression(new std::string("person"), new std::string("age"))));
    yieldColumns->addColumn(
        new YieldColumn(new TagPropertyExpression(new std::string("book"), new std::string("name"))));
    auto *project = Project::make(qctx, gv, yieldColumns.get());
  ...
  // With YIELD const expression
  {
    ...
    yieldColumns->addColumn(new YieldColumn(
        new RelationalExpression(Expression::Kind::kRelGT, new ConstantExpression(1), new ConstantExpression(1))));
    yieldColumns->addColumn(
        new YieldColumn(new TagPropertyExpression(new std::string("person"), new std::string("age"))));
    auto *project = Project::make(qctx, gv, yieldColumns.get());
  ...
  {
    auto qctx = getQCtx("FETCH PROP ON person, book \"1\" YIELD person.name, 1 > 1, book.name, person.age");
    ...
    expr1.set_expr(TagPropertyExpression(new std::string("person"), new std::string("name")).encode());
    storage::cpp2::Expr expr2;
    expr2.set_expr(TagPropertyExpression(new std::string("book"), new std::string("name")).encode());
    storage::cpp2::Expr expr3;
    expr3.set_expr(TagPropertyExpression(new std::string("person"), new std::string("age")).encode());
    auto *gv = GetVertices::make(
        qctx, start, 1, src.get(), std::vector<storage::cpp2::VertexProp>{std::move(personProp), std::move(bookProp)},
        std::vector<storage::cpp2::Expr>{std::move(expr1), std::move(expr2), std::move(expr3)});
  ...
  // With YIELD combine properties
  {
    ...
    yieldColumns->addColumn(new YieldColumn(new ArithmeticExpression(
        Expression::Kind::kAdd, new TagPropertyExpression(new std::string("person"), new std::string("name")),
        new TagPropertyExpression(new std::string("person"), new std::string("age")))));
    auto *project = Project::make(qctx, gv, yieldColumns.get());
    project->setColNames({"VertexID", "(person.name+person.age)"});
  ...
  // With YIELD distinct
  {
    ...
    // data collect
    auto *dataCollect = DataCollect::make(qctx, dedup, DataCollect::CollectKind::kRowBasedMove, {dedup->outputVar()});
  ...
  }
  {
    ...
    yieldColumns->addColumn(
        new YieldColumn(new InputPropertyExpression(new std::string(nebula::kVid)), new std::string("VertexID")));
    yieldColumns->addColumn(new YieldColumn(
        new ArithmeticExpression(Expression::Kind::kAdd, new ConstantExpression(1), new ConstantExpression(1))));
    yieldColumns->addColumn(
        new YieldColumn(new TagPropertyExpression(new std::string("person"), new std::string("name"))));
    yieldColumns->addColumn(
        new YieldColumn(new TagPropertyExpression(new std::string("person"), new std::string("age"))));
