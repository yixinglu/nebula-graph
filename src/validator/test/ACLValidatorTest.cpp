...
TEST_F(ACLValidatorTest, Simple) {
  ...
  {  // with password
    auto root = getPlanRoot(folly::stringPrintf("CREATE USER %s WITH PASSWORD \"%s\"", user, password));
    std::vector<PlanNode::Kind> expectedTop{
        PlanNode::Kind::kCreateUser,
        PlanNode::Kind::kStart,
    };
  ...
  // update user
  {
    auto root = getPlanRoot(folly::stringPrintf("ALTER USER %s WITH PASSWORD \"%s\"", user, password));
    std::vector<PlanNode::Kind> expectedTop{
  ...
  {
    auto root =
        getPlanRoot(folly::stringPrintf("CHANGE PASSWORD %s FROM \"%s\" TO \"%s\"", user, password, newPassword));
  ...
  // grant role
  {
    auto root = getPlanRoot(folly::stringPrintf("GRANT ROLE %s ON %s TO %s", roleTypeName, space, user));
  ...
  // revoke role
  {
    auto root = getPlanRoot(folly::stringPrintf("REVOKE ROLE %s ON %s FROM %s", roleTypeName, space, user));
