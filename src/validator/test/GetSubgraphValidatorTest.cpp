...
TEST_F(GetSubgraphValidatorTest, Base) {
  {
    ...
    std::vector<PlanNode::Kind> expected = {
        PK::kDataCollect, PK::kLoop, PK::kStart, PK::kSubgraph, PK::kGetNeighbors, PK::kStart,
    };
...
TEST_F(GetSubgraphValidatorTest, Input) {
  {
    std::string query = "GO FROM \"1\" OVER like YIELD like._src AS src | GET SUBGRAPH FROM $-.src";
    std::vector<PlanNode::Kind> expected = {
        PK::kDataCollect,  PK::kLoop,    PK::kDedup, PK::kSubgraph,     PK::kProject,
        PK::kGetNeighbors, PK::kProject, PK::kStart, PK::kGetNeighbors, PK::kStart,
    };
    EXPECT_TRUE(checkResult(query, expected));
  }
  {
    std::string query = "$a = GO FROM \"1\" OVER like YIELD like._src AS src; GET SUBGRAPH FROM $a.src";
    std::vector<PlanNode::Kind> expected = {
        PK::kDataCollect,  PK::kLoop,    PK::kDedup, PK::kSubgraph,     PK::kProject,
        PK::kGetNeighbors, PK::kProject, PK::kStart, PK::kGetNeighbors, PK::kStart,
    };
  ...
  {
    std::string query = "GO FROM \"1\" OVER like YIELD like._src AS src | GET SUBGRAPH 0 STEPS FROM $-.src";
    std::vector<PlanNode::Kind> expected = {
        PK::kAggregate, PK::kGetVertices, PK::kDedup, PK::kProject, PK::kProject, PK::kGetNeighbors, PK::kStart,
    };
    EXPECT_TRUE(checkResult(query, expected));
  }
  {
    std::string query = "$a = GO FROM \"1\" OVER like YIELD like._src AS src; GET SUBGRAPH 0 STEPS FROM $a.src";
    std::vector<PlanNode::Kind> expected = {
        PK::kAggregate, PK::kGetVertices, PK::kDedup, PK::kProject, PK::kProject, PK::kGetNeighbors, PK::kStart,
    };
    EXPECT_TRUE(checkResult(query, expected));
...
TEST_F(GetSubgraphValidatorTest, RefNotExist) {
  ...
  {
    std::string query = "GO FROM \"1\" OVER like YIELD $$.person.age AS id | GET SUBGRAPH FROM $-.id";
    auto result = checkResult(query);
    EXPECT_EQ(std::string(result.message()),
              "SemanticError: `$-.id', the srcs should be type of FIXED_STRING, but was`INT'");
  }
  {
    std::string query = "$a = GO FROM \"1\" OVER like YIELD $$.person.age AS ID; GET SUBGRAPH FROM $a.ID";
    auto result = checkResult(query);
    EXPECT_EQ(std::string(result.message()),
              "SemanticError: `$a.ID', the srcs should be type of FIXED_STRING, but was`INT'");
  }
  {
    std::string query = "$a = GO FROM \"1\" OVER like YIELD like._src AS src; GET SUBGRAPH FROM $b.src";
    auto result = checkResult(query);
