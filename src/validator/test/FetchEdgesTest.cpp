...
TEST_F(FetchEdgesValidatorTest, FetchEdgesProp) {
  auto src = std::make_unique<VariablePropertyExpression>(new std::string("_VAR1_"), new std::string(kSrc));
  auto type = std::make_unique<VariablePropertyExpression>(new std::string("_VAR2_"), new std::string(kType));
  auto rank = std::make_unique<VariablePropertyExpression>(new std::string("_VAR3_"), new std::string(kRank));
  auto dst = std::make_unique<VariablePropertyExpression>(new std::string("_VAR4_"), new std::string(kDst));
  {
    ...
    auto *ge = GetEdges::make(qctx, start, 1, src.get(), type.get(), rank.get(), dst.get(), std::move(props), {});
  ...
  // With YIELD
  {
    ...
    expr1.set_expr(EdgePropertyExpression(new std::string("like"), new std::string("start")).encode());
    storage::cpp2::Expr expr2;
    expr2.set_expr(EdgePropertyExpression(new std::string("like"), new std::string("end")).encode());
    exprs.emplace_back(std::move(expr1));
    exprs.emplace_back(std::move(expr2));
    auto *ge = GetEdges::make(qctx, start, 1, src.get(), type.get(), rank.get(), dst.get(), std::move(props),
                              std::move(exprs));
    std::vector<std::string> colNames{std::string("like.") + kSrc, std::string("like.") + kDst,
                                      std::string("like.") + kRank, "like.start", "like.end"};
    ge->setColNames(colNames);
    ...
    yieldColumns->addColumn(
        new YieldColumn(new EdgePropertyExpression(new std::string("like"), new std::string("start"))));
    yieldColumns->addColumn(
        new YieldColumn(new EdgePropertyExpression(new std::string("like"), new std::string("end"))));
    auto *project = Project::make(qctx, filter, yieldColumns.get());
    project->setColNames({std::string("like.") + kSrc, std::string("like.") + kDst, std::string("like.") + kRank,
                          "like.start", "like.end"});
    auto result = Eq(qctx->plan()->root(), project);
    ASSERT_TRUE(result.ok()) << result;
  }
  ...
  // With YIELD combine properties
  {
    ...
    yieldColumns->addColumn(new YieldColumn(new RelationalExpression(
        Expression::Kind::kRelGT, new EdgePropertyExpression(new std::string("like"), new std::string("start")),
        new EdgePropertyExpression(new std::string("like"), new std::string("end")))));
    auto *project = Project::make(qctx, filter, yieldColumns.get());
    project->setColNames({std::string("like.") + kSrc, std::string("like.") + kDst, std::string("like.") + kRank,
                          "(like.start>like.end)"});
