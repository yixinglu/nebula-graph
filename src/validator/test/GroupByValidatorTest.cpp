...
TEST_F(GroupByValidatorTest, TestGroupBy) {
  {
    ...
    std::vector<PlanNode::Kind> expected = {PK::kAggregate, PK::kProject, PK::kGetNeighbors, PK::kStart};
  ...
  }
  {
    ...
    std::vector<PlanNode::Kind> expected = {PK::kProject, PK::kAggregate, PK::kProject, PK::kGetNeighbors, PK::kStart};
    EXPECT_TRUE(checkResult(query, expected));
  }
  {
    std::string query =
    ...
    std::vector<PlanNode::Kind> expected = {PK::kAggregate,   PK::kProject, PK::kLeftJoin,     PK::kProject,
  ...
  }
  {
    std::string query =
        ...
        "BIT_OR(2) AS bit_or, "
        "BIT_XOR(3) AS bit_xor";
    std::vector<PlanNode::Kind> expected = {PK::kProject,  PK::kAggregate,    PK::kProject,
                                            PK::kLeftJoin, PK::kProject,      PK::kGetVertices,
...
TEST_F(GroupByValidatorTest, InvalidTest) {
  ...
  }
  {
    ...
    auto result = checkResult(query);
    EXPECT_EQ(std::string(result.message()), "SemanticError: Only support input and variable in GroupBy sentence.");
  }
  {
    // group input noexist
    std::string query =
    ...
    auto result = checkResult(query);
    EXPECT_EQ(std::string(result.message()), "SemanticError: `$-.start_year', not exist prop `start_year'");
  }
  {
    // group name noexist
    std::string query =
  ...
  }
  {
    ...
    auto result = checkResult(query);
    EXPECT_EQ(std::string(result.message()), "SyntaxError: Could not apply aggregation function on `*` near `SUM'");
  }
  {
    // TODO: move to parser UT
