...
Status TraversalValidator::validateStarts(const VerticesClause* clause, Starts& starts) {
  ...
  if (clause->isRef()) {
    auto* src = clause->ref();
    if (src->kind() != Expression::Kind::kInputProperty && src->kind() != Expression::Kind::kVarProperty) {
    ...
    if (type.value() != SchemaUtil::propTypeToValueType(vidType)) {
      std::stringstream ss;
      ss << "`" << src->toString() << "', the srcs should be type of " << apache::thrift::util::enumNameSafe(vidType)
         << ", but was`" << type.value() << "'";
...
Status TraversalValidator::validateOver(const OverClause* clause, Over& over) {
  ...
  if (clause->isOverAll()) {
    ...
    for (auto edge : edges) {
      ...
      if (!edgeType.ok()) {
        return Status::SemanticError("`%s' not found in space [`%s'].", edge.c_str(), space_.name.c_str());
      }
    ...
    for (auto* edge : edges) {
      ...
      if (!edgeType.ok()) {
        return Status::SemanticError("%s not found in space [%s].", edgeName.c_str(), space_.name.c_str());
      }
      over.edgeTypes.emplace_back(edgeType.value());
    }
  }
...
void TraversalValidator::buildConstantInput(Starts& starts, std::string& startVidsVar) {
  ...
  starts.src = new VariablePropertyExpression(new std::string(startVidsVar), new std::string(kVid));
...
Expression* TraversalValidator::buildNStepLoopCondition(uint32_t steps) const {
  ...
  return qctx_->objPool()->add(new RelationalExpression(
      ...
      new UnaryExpression(Expression::Kind::kUnaryIncr, new VariableExpression(new std::string(loopSteps_))),
