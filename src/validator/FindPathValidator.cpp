...
void FindPathValidator::buildStart(Starts& starts, std::string& startVidsVar, bool reverse) {
...
Status FindPathValidator::singlePairPlan() {
  ...
  auto* conjunct = ConjunctPath::make(qctx_, forward, backward, ConjunctPath::PathKind::kBiBFS, steps_.steps);
  conjunct->setColNames({"_path"});

  auto* loop = Loop::make(qctx_, nullptr, conjunct, buildBfsLoopCondition(steps_.steps, conjunct->outputVar()));

  auto* dataCollect = DataCollect::make(qctx_, loop, DataCollect::CollectKind::kBFSShortest, {conjunct->outputVar()});
...
Expression* FindPathValidator::buildBfsLoopCondition(uint32_t steps, const std::string& pathVar) {
  ...
  auto* pathEmpty = new RelationalExpression(
      Expression::Kind::kRelEQ, new FunctionCallExpression(new std::string("size"), args), new ConstantExpression(0));
  auto* notFoundPath = new LogicalExpression(
      Expression::Kind::kLogicalOr,
      new RelationalExpression(Expression::Kind::kRelEQ, new VariableExpression(new std::string(pathVar)),
                               new ConstantExpression(Value())),
      pathEmpty);
  return qctx_->objPool()->add(new LogicalExpression(Expression::Kind::kLogicalAnd, nSteps, notFoundPath));
}

void FindPathValidator::buildEdgeProps(GetNeighbors::EdgeProps& edgeProps, bool reverse, bool isInEdge) {
...
PlanNode* FindPathValidator::buildAllPairFirstDataSet(PlanNode* dep, const std::string& inputVar) {
  auto* vid = new YieldColumn(new VariablePropertyExpression(new std::string("*"), new std::string(kVid)),
                              new std::string(kVid));
  auto* pathExpr = new PathBuildExpression();
  pathExpr->add(std::make_unique<VariablePropertyExpression>(new std::string("*"), new std::string(kVid)));
...
Status FindPathValidator::allPairPaths() {
  ...
  auto* conjunct = ConjunctPath::make(qctx_, forward, backward, ConjunctPath::PathKind::kAllPaths, steps_.steps);
  conjunct->setColNames({"_path"});
  ...
  auto* loop = Loop::make(qctx_, projectTo, conjunct, buildAllPathsLoopCondition(steps_.steps));

  auto* dataCollect = DataCollect::make(qctx_, loop, DataCollect::CollectKind::kAllPaths, {conjunct->outputVar()});
  dataCollect->setColNames({"path"});
...
PlanNode* FindPathValidator::allPaths(PlanNode* dep, Starts& starts, std::string& startVidsVar, bool reverse) {
  auto* gn = GetNeighbors::make(qctx_, dep, space_.id);
  gn->setSrc(starts.src);
  gn->setEdgeProps(buildEdgeKey(reverse));
  gn->setInputVar(startVidsVar);
...
PlanNode* FindPathValidator::buildMultiPairFirstDataSet(PlanNode* dep, const std::string& inputVar,
                                                        const std::string& outputVar) {
  auto* dst = new YieldColumn(new VariablePropertyExpression(new std::string("*"), new std::string(kVid)),
                              new std::string(kDst));
  auto* src = new YieldColumn(new VariablePropertyExpression(new std::string("*"), new std::string(kVid)),
                              new std::string(kSrc));
...
Status FindPathValidator::multiPairPlan() {
  ...
  auto* conjunct = ConjunctPath::make(qctx_, forward, backward, ConjunctPath::PathKind::kFloyd, steps_.steps);
  ...
  auto* loop = Loop::make(qctx_, cartesianProduct, conjunct,
                          buildMultiPairLoopCondition(steps_.steps, cartesianProduct->outputVar()));

  auto* dataCollect =
      DataCollect::make(qctx_, loop, DataCollect::CollectKind::kMultiplePairShortest, {conjunct->outputVar()});
  dataCollect->setColNames({"path"});
...
PlanNode* FindPathValidator::multiPairShortestPath(PlanNode* dep, Starts& starts, std::string& startVidsVar,
                                                   std::string& pathVar, bool reverse) {
  auto* gn = GetNeighbors::make(qctx_, dep, space_.id);
  gn->setSrc(starts.src);
  gn->setEdgeProps(buildEdgeKey(reverse));
  gn->setInputVar(startVidsVar);
  ...
  auto* columns = qctx_->objPool()->add(new YieldColumns());
  auto* column = new YieldColumn(new VariablePropertyExpression(new std::string("*"), new std::string(kDst)),
                                 new std::string(kVid));
...
Expression* FindPathValidator::buildMultiPairLoopCondition(uint32_t steps, std::string conditionalVar) {
  ...
  auto* notAllPathFind = new RelationalExpression(
      Expression::Kind::kRelGT, new FunctionCallExpression(new std::string("size"), args), new ConstantExpression(0));

  auto* terminationCondition =
      new LogicalExpression(Expression::Kind::kLogicalOr,
                            new RelationalExpression(Expression::Kind::kRelEQ, new ConstantExpression(isWeight_),
                                                     new ConstantExpression(true)),
                            notAllPathFind);
  auto* condition = new LogicalExpression(Expression::Kind::kLogicalAnd, nSteps, terminationCondition);
