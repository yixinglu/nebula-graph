...
Status CreateUserValidator::validateImpl() {
  ...
  if (sentence->getAccount()->size() > kUsernameMaxLength) {
    return Status::SemanticError("Username exceed maximum length %ld characters.", kUsernameMaxLength);
  }
  if (sentence->getPassword()->size() > kPasswordMaxLength) {
    return Status::SemanticError("Password exceed maximum length %ld characters.", kPasswordMaxLength);
  }
  return Status::OK();
}
...
Status CreateUserValidator::toPlan() {
  ...
  return genSingleNodePlan<CreateUser>(sentence->getAccount(), sentence->getPassword(), sentence->ifNotExists());
}
...
Status ChangePasswordValidator::validateImpl() {
  ...
  if (sentence->getOldPwd()->size() > kPasswordMaxLength) {
    return Status::SemanticError("Old password exceed maximum length %ld characters.", kPasswordMaxLength);
  }
  if (sentence->getNewPwd()->size() > kPasswordMaxLength) {
    return Status::SemanticError("New password exceed maximum length %ld characters.", kPasswordMaxLength);
  }
  return Status::OK();
}
...
Status ChangePasswordValidator::toPlan() {
  ...
  return genSingleNodePlan<ChangePassword>(sentence->getAccount(), sentence->getOldPwd(), sentence->getNewPwd());
}
...
Status GrantRoleValidator::toPlan() {
  ...
  return genSingleNodePlan<GrantRole>(sentence->getAccount(), sentence->getAclItemClause()->getSpaceName(),
...
Status RevokeRoleValidator::toPlan() {
  ...
  return genSingleNodePlan<RevokeRole>(sentence->getAccount(), sentence->getAclItemClause()->getSpaceName(),
                                       sentence->getAclItemClause()->getRoleType());
}
