...
Status GoValidator::validateImpl() {
  ...
  if (!exprProps_.varProps().empty() && from_.fromType != kVariable) {
    return Status::SemanticError("A variable must be referred in FROM before used in WHERE or YIELD");
  }

  if ((!exprProps_.inputProps().empty() && !exprProps_.varProps().empty()) || exprProps_.varProps().size() > 1) {
...
Status GoValidator::validateWhere(WhereClause* where) {
  ...
  if (type != Value::Type::BOOL && type != Value::Type::NULLVALUE && type != Value::Type::__EMPTY__) {
    std::stringstream ss;
  ...
  if (cols.empty() && over_.isOverAll) {
    ...
    for (auto col : cols) {
      ...
      if (graph::ExpressionUtils::findAny(colExpr, {Expression::Kind::kAggregate})) {
        return Status::SemanticError("`%s', not support aggregate function in go sentence.", col->toString().c_str());
      }
      auto colName = deduceColName(col);
      colNames_.emplace_back(colName);
...
Status GoValidator::oneStep(PlanNode* dependencyForGn, const std::string& inputVarNameForGN,
  ...
  if (!exprProps_.inputProps().empty() || !exprProps_.varProps().empty() || !exprProps_.dstTagProps().empty() ||
      from_.fromType != FromType::kInstantExpr) {
  ...
  if (from_.fromType != FromType::kInstantExpr) {
    joinInput =
        buildJoinPipeOrVariableInput(projectFromJoin, joinDstProps == nullptr ? projectSrcEdgeProps : joinDstProps);
  }
  ...
  if (filter_ != nullptr) {
    auto* filterNode = Filter::make(qctx_, dependencyForProjectResult, newFilter_ != nullptr ? newFilter_ : filter_);
  ...
  auto* projectResult =
      Project::make(qctx_, dependencyForProjectResult, newYieldCols_ != nullptr ? newYieldCols_ : yields_);
...
Status GoValidator::buildNStepsPlan() {
  ...
  if (from_.fromType != FromType::kInstantExpr && projectLeftVarForJoin != nullptr && dedupSrcDstVids != nullptr) {
    projectFromJoin = traceToStartVid(projectLeftVarForJoin, dedupSrcDstVids);
    loopBody = projectFromJoin;
  }

  auto* loop = Loop::make(qctx_,
...
Status GoValidator::buildMToNPlan() {
  ...
  if (filter_ != nullptr) {
    auto* filterNode = Filter::make(qctx_, dependencyForProjectResult, newFilter_ != nullptr ? newFilter_ : filter_);
    if (dependencyForProjectResult == dedupDstVids || dependencyForProjectResult == dedupSrcDstVids) {
  ...
  auto* dataCollect = DataCollect::make(qctx_, loop, DataCollect::CollectKind::kMToN, collectVars);
...
PlanNode* GoValidator::buildProjectSrcEdgePropsForGN(std::string gnVar, PlanNode* dependency) {
  ...
  if (from_.fromType != FromType::kInstantExpr) {
    auto* srcVidCol = new YieldColumn(new VariablePropertyExpression(new std::string(gnVar), new std::string(kVid)),
                                      new std::string(kVid));
  ...
  if (!exprProps_.dstTagProps().empty()) {
    ...
    auto* dstVidCol = new YieldColumn(new EdgePropertyExpression(new std::string("*"), new std::string(kDst)),
...
PlanNode* GoValidator::buildJoinDstProps(PlanNode* projectSrcDstProps) {
  ...
  auto* vids = objPool->makeAndAdd<VariablePropertyExpression>(new std::string(projectSrcDstProps->outputVar()),
                                                               new std::string(joinDstVidColName_));
  auto* getDstVertices = GetVertices::make(qctx_, projectSrcDstProps, space_.id, vids, buildDstVertexProps(), {});
  ...
  auto* vidCol = new YieldColumn(
      new VariablePropertyExpression(new std::string(getDstVertices->outputVar()), new std::string(kVid)),
  ...
  auto* joinHashKey = objPool->makeAndAdd<VariablePropertyExpression>(new std::string(projectSrcDstProps->outputVar()),
                                                                      new std::string(joinDstVidColName_));
  auto* probeKey = objPool->makeAndAdd<VariablePropertyExpression>(new std::string(project->outputVar()),
                                                                   new std::string(vidColName));
  auto joinDst = LeftJoin::make(qctx_, project, {projectSrcDstProps->outputVar(), ExecutionContext::kLatestVersion},
                                {project->outputVar(), ExecutionContext::kLatestVersion}, {joinHashKey}, {probeKey});
  VLOG(1) << joinDst->outputVar() << " hash key: " << joinHashKey->toString() << " probe key: " << probeKey->toString();
...
PlanNode* GoValidator::buildJoinPipeOrVariableInput(PlanNode* projectFromJoin, PlanNode* dependencyForJoinInput) {
  auto* pool = qctx_->objPool();

  if (steps_.steps > 1 || steps_.mToN != nullptr) {
    DCHECK(projectFromJoin != nullptr);
    auto* joinHashKey = pool->add(
        new VariablePropertyExpression(new std::string(dependencyForJoinInput->outputVar()), new std::string(kVid)));
    auto* probeKey = pool->add(
        new VariablePropertyExpression(new std::string(projectFromJoin->outputVar()), new std::string(dstVidColName_)));
    auto* join = LeftJoin::make(
        qctx_, dependencyForJoinInput, {dependencyForJoinInput->outputVar(), ExecutionContext::kLatestVersion},
        {projectFromJoin->outputVar(),
         steps_.mToN != nullptr ? ExecutionContext::kPreviousOneVersion : ExecutionContext::kLatestVersion},
  ...
  auto* joinHashKey = pool->add(new VariablePropertyExpression(
      new std::string(dependencyForJoinInput->outputVar()),
      new std::string((steps_.steps > 1 || steps_.mToN != nullptr) ? from_.firstBeginningSrcVidColName : kVid)));
  std::string varName = from_.fromType == kPipe ? inputVarName_ : from_.userDefinedVarName;
  auto* joinInput = LeftJoin::make(qctx_, dependencyForJoinInput,
                                   {dependencyForJoinInput->outputVar(), ExecutionContext::kLatestVersion},
                                   {varName, ExecutionContext::kLatestVersion}, {joinHashKey}, {from_.originalSrc});
  std::vector<std::string> colNames = dependencyForJoinInput->colNames();
...
PlanNode* GoValidator::traceToStartVid(PlanNode* projectLeftVarForJoin, PlanNode* dedupDstVids) {
  ...
  auto* pool = qctx_->objPool();
  auto hashKey = new VariablePropertyExpression(new std::string(projectLeftVarForJoin->outputVar()),
                                                new std::string(dstVidColName_));
  pool->add(hashKey);
  auto probeKey =
      new VariablePropertyExpression(new std::string(dedupDstVids->outputVar()), new std::string(srcVidColName_));
  pool->add(probeKey);
  auto* join =
      LeftJoin::make(qctx_, dedupDstVids, {projectLeftVarForJoin->outputVar(), ExecutionContext::kLatestVersion},
                     {dedupDstVids->outputVar(), ExecutionContext::kLatestVersion}, {hashKey}, {probeKey});
  ...
  auto* column = new YieldColumn(new InputPropertyExpression(new std::string(from_.firstBeginningSrcVidColName)),
                                 new std::string(from_.firstBeginningSrcVidColName));
  columns->addColumn(column);
  column = new YieldColumn(new InputPropertyExpression(new std::string(kVid)), new std::string(dstVidColName_));
  columns->addColumn(column);
...
PlanNode* GoValidator::buildLeftVarForTraceJoin(PlanNode* dedupStartVid) {
  ...
  auto* columns = pool->add(new YieldColumns());
  auto* column =
      new YieldColumn(from_.originalSrc->clone().release(), new std::string(from_.firstBeginningSrcVidColName));
  columns->addColumn(column);
  ...
  projectLeftVarForJoin->setInputVar(from_.fromType == kPipe ? inputVarName_ : from_.userDefinedVarName);
...
GetNeighbors::VertexProps GoValidator::buildSrcVertexProps() {
  ...
  if (!exprProps_.srcTagProps().empty()) {
    vertexProps = std::make_unique<std::vector<storage::cpp2::VertexProp>>(exprProps_.srcTagProps().size());
    std::transform(exprProps_.srcTagProps().begin(), exprProps_.srcTagProps().end(), vertexProps->begin(),
                   [](auto& tag) {
...
std::vector<storage::cpp2::VertexProp> GoValidator::buildDstVertexProps() {
  ...
  if (!exprProps_.dstTagProps().empty()) {
    std::transform(exprProps_.dstTagProps().begin(), exprProps_.dstTagProps().end(), vertexProps.begin(),
                   [](auto& tag) {
                     storage::cpp2::VertexProp vp;
                     vp.set_tag(tag.first);
                     std::vector<std::string> props(tag.second.begin(), tag.second.end());
...
GetNeighbors::EdgeProps GoValidator::buildEdgeProps() {
  ...
  bool onlyInputPropsOrConstant =
      exprProps_.srcTagProps().empty() && exprProps_.dstTagProps().empty() && exprProps_.edgeProps().empty();
...
GetNeighbors::EdgeProps GoValidator::buildEdgeDst() {
  ...
  bool onlyInputPropsOrConstant =
      exprProps_.srcTagProps().empty() && exprProps_.dstTagProps().empty() && exprProps_.edgeProps().empty();
  if (!exprProps_.edgeProps().empty() || !exprProps_.dstTagProps().empty() || onlyInputPropsOrConstant) {
    if (over_.direction == storage::cpp2::EdgeDirection::IN_EDGE) {
      edgeProps = std::make_unique<std::vector<storage::cpp2::EdgeProp>>(over_.edgeTypes.size());
      std::transform(over_.edgeTypes.begin(), over_.edgeTypes.end(), edgeProps->begin(), [](auto& type) {
        storage::cpp2::EdgeProp ep;
        ep.set_type(-type);
        ep.set_props({kDst});
        return ep;
      ...
      edgeProps = std::make_unique<std::vector<storage::cpp2::EdgeProp>>(over_.edgeTypes.size() * 2);
      std::transform(over_.edgeTypes.begin(), over_.edgeTypes.end(), edgeProps->begin(), [](auto& type) {
        storage::cpp2::EdgeProp ep;
        ep.set_type(type);
        ep.set_props({kDst});
      ...
      std::transform(over_.edgeTypes.begin(), over_.edgeTypes.end(), edgeProps->begin() + over_.edgeTypes.size(),
                     [](auto& type) {
                       storage::cpp2::EdgeProp ep;
                       ep.set_type(-type);
                       ep.set_props({kDst});
...
void GoValidator::extractPropExprs(const Expression* expr) {
  ExtractPropExprVisitor visitor(vctx_, srcAndEdgePropCols_, dstPropCols_, inputPropCols_, propExprColMap_);
...
Status GoValidator::buildColumns() {
  if (exprProps_.dstTagProps().empty() && exprProps_.inputProps().empty() && exprProps_.varProps().empty() &&
      from_.fromType == FromType::kInstantExpr) {
    return Status::OK();
  }
...
PlanNode* GoValidator::projectSrcDstVidsFromGN(PlanNode* dep, PlanNode* gn) {
  ...
  auto* column =
      new YieldColumn(new EdgePropertyExpression(new std::string("*"), new std::string(kDst)), new std::string(kVid));
  columns->addColumn(column);

  srcVidColName_ = vctx_->anonColGen()->getCol();
  column = new YieldColumn(new InputPropertyExpression(new std::string(kVid)), new std::string(srcVidColName_));
  columns->addColumn(column);
