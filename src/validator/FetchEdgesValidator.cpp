...
Status FetchEdgesValidator::toPlan() {
  ...
  auto *getEdgesNode = GetEdges::make(qctx_, nullptr, spaceId_, src_, type_, rank_, dst_, std::move(props_),
                                      std::move(exprs_), dedup_, limit_, std::move(orderBy_), std::move(filter_));
...
Status FetchEdgesValidator::preparePropertiesWithYield(const YieldClause *yield) {
  ...
  newYieldColumns->addColumn(new YieldColumn(new EdgeSrcIdExpression(new std::string(edgeTypeName_))));
  newYieldColumns->addColumn(new YieldColumn(new EdgeDstIdExpression(new std::string(edgeTypeName_))));
  newYieldColumns->addColumn(new YieldColumn(new EdgeRankExpression(new std::string(edgeTypeName_))));
  ...
  for (auto col : newYield_->columns()) {
    ...
    if (invalidExpr != nullptr) {
      return Status::SemanticError("Invalid newYield_ expression `%s'.", col->expr()->toString().c_str());
    }
...
const Expression *FetchEdgesValidator::findInvalidYieldExpression(const Expression *root) {
  return ExpressionUtils::findAny(root, {Expression::Kind::kInputProperty, Expression::Kind::kVarProperty,
                                         Expression::Kind::kSrcProperty, Expression::Kind::kDstProperty});
}
...
std::string FetchEdgesValidator::buildConstantInput() {
  ...
  src_ = pool->makeAndAdd<VariablePropertyExpression>(new std::string(input), new std::string(kSrc));
  type_ = pool->makeAndAdd<ConstantExpression>(edgeType_);
  rank_ = pool->makeAndAdd<VariablePropertyExpression>(new std::string(input), new std::string(kRank));
  dst_ = pool->makeAndAdd<VariablePropertyExpression>(new std::string(input), new std::string(kDst));
  return input;
}
...
Expression *FetchEdgesValidator::emptyEdgeKeyFilter() {
  ...
  auto *edgeKeyNotEmptyExpr = qctx_->objPool()->add(lgAnd(srcNotEmptyExpr, lgAnd(dstNotEmptyExpr, rankNotEmptyExpr)));
