...
Status FetchVerticesValidator::toPlan() {
  ...
  auto *getVerticesNode = GetVertices::make(qctx_, nullptr, space_.id, src_, std::move(props_), std::move(exprs_),
                                            dedup_, std::move(orderBy_), limit_, std::move(filter_));
...
Status FetchVerticesValidator::prepareVertices() {
  ...
  for (const auto vid : vids) {
    ...
    if (v.type() != vidType_) {
      std::stringstream ss;
      ss << "`" << vid->toString() << "', the vid should be type of " << vidType_ << ", but was`" << v.type() << "'";
...
std::string FetchVerticesValidator::buildConstantInput() {
  ...
  src_ = qctx_->objPool()->makeAndAdd<VariablePropertyExpression>(new std::string(input), new std::string(kVid));
  return input;
}
