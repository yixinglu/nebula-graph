...
Status SetValidator::validateImpl() {
  ...
  for (size_t i = 0, e = lCols.size(); i < e; i++) {
    if (lCols[i].name != rCols[i].name) {
      return Status::SemanticError("different column names to UNION/INTERSECT/MINUS are not supported");
    }
  }
...
Status SetValidator::toPlan() {
  ...
  switch (setSentence->op()) {
    ...
    default:
      return Status::SemanticError("Unknown operator: %ld", static_cast<int64_t>(setSentence->op()));
  }
