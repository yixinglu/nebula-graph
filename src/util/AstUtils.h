...
class AstUtils final {
 public:
  ...
  static Status reprAstCheck(const Sentence& origin) {
    ...
    if (!copyResult.ok()) {
      return Status::Error("The repr sentence `%s' can't be parsed, error: `%s'.", toString.c_str(),
                           copyResult.status().toString().c_str());
    }
    auto copyToString = copyResult.value()->toString();
    if (toString != copyToString) {
      return Status::Error("The reparsed ast `%s' is different from origin `%s'.", copyToString.c_str(),
                           toString.c_str());
    }
    return Status::OK();
  }
