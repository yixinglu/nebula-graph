...
// static
Status SchemaUtil::validateProps(const std::vector<SchemaPropItem *> &schemaProps, meta::cpp2::Schema &schema) {
...
// static
std::shared_ptr<const meta::NebulaSchemaProvider> SchemaUtil::generateSchemaProvider(const SchemaVer ver,
  ...
  for (auto col : schema.get_columns()) {
    ...
    schemaPtr->addField(col.get_name(), col.get_type().get_type(), col.type.type_length_ref().value_or(0),
                        col.nullable_ref().value_or(false), hasDef ? defaultValueExpr.release() : nullptr);
  }
  return schemaPtr;
}
...
Status SchemaUtil::setTTLCol(SchemaPropItem *schemaProp, meta::cpp2::Schema &schema) {
  ...
  for (auto &col : *schema.columns_ref()) {
    if (col.name == ttlColName) {
      ...
      if (col.type.type != meta::cpp2::PropertyType::INT64 && col.type.type != meta::cpp2::PropertyType::TIMESTAMP) {
...
StatusOr<DataSet> SchemaUtil::toShowCreateSchema(bool isTag, const std::string &name,
                                                 const meta::cpp2::Schema &schema) {
  DataSet dataSet;
...
std::string SchemaUtil::typeToString(const meta::cpp2::ColumnDef &col) {
  ...
  std::transform(std::begin(str), std::end(str), std::begin(str), [](uint8_t c) { return std::tolower(c); });
  return str;
}
