...
class ExpressionUtils {
 public:
  ...
  static inline bool isKindOf(const Expression* expr, const std::unordered_set<Expression::Kind>& expected) {
    return expected.find(expr->kind()) != expected.end();
  }

  // null for not found
  static const Expression* findAny(const Expression* self, const std::unordered_set<Expression::Kind>& expected) {
  ...
  static std::vector<const Expression*> collectAll(const Expression* self,
  ...
  static std::vector<const Expression*> findAllStorage(const Expression* expr) {
    return collectAll(expr, {Expression::Kind::kTagProperty, Expression::Kind::kEdgeProperty,
                             Expression::Kind::kDstProperty, Expression::Kind::kSrcProperty, Expression::Kind::kEdgeSrc,
                             Expression::Kind::kEdgeType, Expression::Kind::kEdgeRank, Expression::Kind::kEdgeDst,
  ...
  static bool isConstExpr(const Expression* expr) {
    return !hasAny(expr,
                   {Expression::Kind::kInputProperty, Expression::Kind::kVarProperty, Expression::Kind::kVar,
                    Expression::Kind::kVersionedVar, Expression::Kind::kLabelAttribute, Expression::Kind::kTagProperty,
                    Expression::Kind::kEdgeProperty, Expression::Kind::kDstProperty, Expression::Kind::kSrcProperty,
                    Expression::Kind::kEdgeSrc, Expression::Kind::kEdgeType, Expression::Kind::kEdgeRank,
                    Expression::Kind::kEdgeDst, Expression::Kind::kVertex, Expression::Kind::kEdge});
  }

  // rewrite LabelAttr to tagProp  (just for nGql)
  static Expression* rewriteLabelAttr2TagProp(const Expression* expr) {
    auto matcher = [](const Expression* e) -> bool { return e->kind() == Expression::Kind::kLabelAttribute; };
  ...
  static Expression* rewriteAgg2VarProp(const Expression* expr) {
    auto matcher = [](const Expression* e) -> bool { return e->kind() == Expression::Kind::kAggregate; };
    auto rewriter = [](const Expression* e) -> Expression* {
      return new VariablePropertyExpression(new std::string(""), new std::string(e->toString()));
    };

    return RewriteVisitor::transform(
        expr, std::move(matcher), std::move(rewriter),
        {Expression::Kind::kFunctionCall, Expression::Kind::kTypeCasting, Expression::Kind::kAdd,
         Expression::Kind::kMinus, Expression::Kind::kMultiply, Expression::Kind::kDivision, Expression::Kind::kMod});
  }
  ...
  static void pullAndsImpl(LogicalExpression* expr, std::vector<std::unique_ptr<Expression>>& operands);

  static Expression* pullOrs(Expression* expr);
  static void pullOrsImpl(LogicalExpression* expr, std::vector<std::unique_ptr<Expression>>& operands);

  static VariablePropertyExpression* newVarPropExpr(const std::string& prop, const std::string& var = "");

  static std::unique_ptr<InputPropertyExpression> inputPropExpr(const std::string& prop);

  static std::unique_ptr<Expression> pushOrs(const std::vector<std::unique_ptr<Expression>>& rels);

  static std::unique_ptr<Expression> pushAnds(const std::vector<std::unique_ptr<Expression>>& rels);

  static std::unique_ptr<Expression> pushImpl(Expression::Kind kind,
