...
TEST_F(ExpressionUtilsTest, CheckComponent) {
  {
    ...
    ASSERT_TRUE(ExpressionUtils::isKindOf(root.get(), {Expression::Kind::kConstant, Expression::Kind::kAdd}));
    ASSERT_TRUE(ExpressionUtils::hasAny(root.get(), {Expression::Kind::kConstant, Expression::Kind::kAdd}));
    ...
    ASSERT_FALSE(ExpressionUtils::isKindOf(root.get(), {Expression::Kind::kDivision, Expression::Kind::kAdd}));
    ASSERT_FALSE(ExpressionUtils::hasAny(root.get(), {Expression::Kind::kDstProperty, Expression::Kind::kAdd}));

    // find
    const Expression *found = ExpressionUtils::findAny(root.get(), {Expression::Kind::kConstant});
    ASSERT_EQ(found, root.get());

    found = ExpressionUtils::findAny(
        root.get(), {Expression::Kind::kConstant, Expression::Kind::kAdd, Expression::Kind::kEdgeProperty});
    ASSERT_EQ(found, root.get());

    found = ExpressionUtils::findAny(root.get(), {Expression::Kind::kEdgeDst});
    ASSERT_EQ(found, nullptr);

    found = ExpressionUtils::findAny(root.get(), {Expression::Kind::kEdgeRank, Expression::Kind::kInputProperty});
    ASSERT_EQ(found, nullptr);

    // find all
    const auto willFoundAll = std::vector<const Expression *>{root.get()};
    std::vector<const Expression *> founds = ExpressionUtils::collectAll(root.get(), {Expression::Kind::kConstant});
    ASSERT_EQ(founds, willFoundAll);

    founds = ExpressionUtils::collectAll(
        root.get(), {Expression::Kind::kAdd, Expression::Kind::kConstant, Expression::Kind::kEdgeDst});
    ASSERT_EQ(founds, willFoundAll);
    ...
    founds = ExpressionUtils::collectAll(
        root.get(), {Expression::Kind::kUnaryNegate, Expression::Kind::kEdgeDst, Expression::Kind::kEdgeDst});
    ASSERT_TRUE(founds.empty());
  }

  {
    ...
    ASSERT_TRUE(ExpressionUtils::isKindOf(root.get(), {Expression::Kind::kTypeCasting, Expression::Kind::kAdd}));
    ASSERT_TRUE(ExpressionUtils::hasAny(root.get(), {Expression::Kind::kTypeCasting, Expression::Kind::kAdd}));
    ...
    // found
    const Expression *found = ExpressionUtils::findAny(root.get(), {Expression::Kind::kTypeCasting});
    ASSERT_EQ(found, root.get());

    found = ExpressionUtils::findAny(
        root.get(), {Expression::Kind::kFunctionCall, Expression::Kind::kTypeCasting, Expression::Kind::kLogicalAnd});
    ASSERT_EQ(found, root.get());
    ...
    found = ExpressionUtils::findAny(
        root.get(), {Expression::Kind::kLogicalXor, Expression::Kind::kRelGE, Expression::Kind::kEdgeProperty});
    ASSERT_EQ(found, nullptr);
    ...
    founds = ExpressionUtils::collectAll(root.get(), {Expression::Kind::kFunctionCall, Expression::Kind::kTypeCasting});
    ASSERT_EQ(founds.size(), 3);

    founds = ExpressionUtils::collectAll(root.get(), {Expression::Kind::kAdd});
    ASSERT_TRUE(founds.empty());

    founds = ExpressionUtils::collectAll(root.get(), {Expression::Kind::kRelLE, Expression::Kind::kDstProperty});
    ASSERT_TRUE(founds.empty());
  }

  {
    ...
    const auto root = std::make_unique<ArithmeticExpression>(
        ...
        new ArithmeticExpression(
            Expression::Kind::kDivision, new ConstantExpression(3),
            new ArithmeticExpression(Expression::Kind::kMinus, new ConstantExpression(4), new ConstantExpression(2))),
        new ArithmeticExpression(Expression::Kind::kMod,
                                 new ArithmeticExpression(Expression::Kind::kMultiply, new ConstantExpression(3),
    ...
    ASSERT_TRUE(ExpressionUtils::isKindOf(root.get(), {Expression::Kind::kTypeCasting, Expression::Kind::kAdd}));
    ASSERT_TRUE(ExpressionUtils::hasAny(root.get(), {Expression::Kind::kLabelAttribute, Expression::Kind::kDivision}));
    ...
    ASSERT_FALSE(ExpressionUtils::isKindOf(root.get(), {Expression::Kind::kDivision, Expression::Kind::kEdgeProperty}));
    ASSERT_FALSE(ExpressionUtils::hasAny(root.get(), {Expression::Kind::kDstProperty, Expression::Kind::kLogicalAnd}));
    ...
    found = ExpressionUtils::findAny(
        root.get(), {Expression::Kind::kFunctionCall, Expression::Kind::kRelLE, Expression::Kind::kMultiply});
    ...
    found = ExpressionUtils::findAny(
        root.get(), {Expression::Kind::kLogicalXor, Expression::Kind::kEdgeRank, Expression::Kind::kUnaryNot});
    ASSERT_EQ(found, nullptr);
    ...
    founds = ExpressionUtils::collectAll(root.get(), {Expression::Kind::kDivision, Expression::Kind::kMinus});
    ASSERT_EQ(founds.size(), 2);

    founds = ExpressionUtils::collectAll(root.get(), {Expression::Kind::kEdgeDst});
    ASSERT_TRUE(founds.empty());

    founds = ExpressionUtils::collectAll(root.get(), {Expression::Kind::kLogicalAnd, Expression::Kind::kUnaryNegate});
    ASSERT_TRUE(founds.empty());
  }
}
...
TEST_F(ExpressionUtilsTest, PullAnds) {
  ...
  // true AND false
  {
    ...
    LogicalExpression expected(Kind::kLogicalAnd, first->clone().release(), second->clone().release());
  ...
  // true AND false AND true
  {
    ...
    LogicalExpression expr(Kind::kLogicalAnd, new LogicalExpression(Kind::kLogicalAnd, first, second), third);
  ...
  // true AND (false AND true)
  {
    ...
    auto *second = new ConstantExpression(false);
    auto *third = new ConstantExpression(true);
    LogicalExpression expr(Kind::kLogicalAnd, first, new LogicalExpression(Kind::kLogicalAnd, second, third));
    LogicalExpression expected(Kind::kLogicalAnd);
    expected.addOperand(first->clone().release());
    expected.addOperand(second->clone().release());
    expected.addOperand(third->clone().release());
  ...
  {
    auto *first = new LogicalExpression(Kind::kLogicalOr, new ConstantExpression(true), new ConstantExpression(false));
    auto *second = new LogicalExpression(Kind::kLogicalOr, new ConstantExpression(true), new ConstantExpression(false));
    LogicalExpression expr(Kind::kLogicalAnd, first, second);
    LogicalExpression expected(Kind::kLogicalAnd);
    expected.addOperand(first->clone().release());
    expected.addOperand(second->clone().release());
  ...
  {
    auto *first = new ConstantExpression(true);
    auto *second = new LogicalExpression(
        Kind::kLogicalOr,
        new LogicalExpression(Kind::kLogicalAnd, new ConstantExpression(false), new ConstantExpression(true)),
...
TEST_F(ExpressionUtilsTest, PullOrs) {
  ...
  // true OR false
  {
    ...
    LogicalExpression expected(Kind::kLogicalOr, first->clone().release(), second->clone().release());
  ...
  // true OR false OR true
  {
    ...
    auto *second = new ConstantExpression(false);
    auto *third = new ConstantExpression(true);
    LogicalExpression expr(Kind::kLogicalOr, new LogicalExpression(Kind::kLogicalOr, first, second), third);
  ...
  // true OR (false OR true)
  {
    ...
    auto *second = new ConstantExpression(false);
    auto *third = new ConstantExpression(true);
    LogicalExpression expr(Kind::kLogicalOr, first, new LogicalExpression(Kind::kLogicalOr, second, third));
    LogicalExpression expected(Kind::kLogicalOr);
    expected.addOperand(first->clone().release());
    expected.addOperand(second->clone().release());
    expected.addOperand(third->clone().release());
  ...
  {
    auto *first = new LogicalExpression(Kind::kLogicalAnd, new ConstantExpression(true), new ConstantExpression(false));
    auto *second =
        new LogicalExpression(Kind::kLogicalAnd, new ConstantExpression(true), new ConstantExpression(false));
    LogicalExpression expr(Kind::kLogicalOr, first, second);
    LogicalExpression expected(Kind::kLogicalOr, first->clone().release(), second->clone().release());
    ExpressionUtils::pullOrs(&expr);
    ASSERT_EQ(expected, expr);
  ...
  // true OR ((false OR true) AND false) OR true
  {
    ...
    auto *second = new LogicalExpression(
        Kind::kLogicalAnd,
        new LogicalExpression(Kind::kLogicalOr, new ConstantExpression(false), new ConstantExpression(true)),
        new ConstantExpression(false));
    auto *third = new ConstantExpression(true);
    LogicalExpression expr(Kind::kLogicalOr, new LogicalExpression(Kind::kLogicalOr, first, second), third);
    LogicalExpression expected(Kind::kLogicalOr);
