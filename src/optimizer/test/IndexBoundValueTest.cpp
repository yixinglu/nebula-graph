...
TEST(IndexBoundValueTest, DoubleTest) {
  ...
  EXPECT_EQ(5.1 - 0.0000000000000001, OptimizerUtils::boundValue(col, OP::LESS_THAN, Value(5.1)));

  EXPECT_EQ(-(5.1 + 0.0000000000000001), OptimizerUtils::boundValue(col, OP::LESS_THAN, Value(-5.1)));

  EXPECT_EQ(maxDouble, OptimizerUtils::boundValue(col, OP::GREATER_THAN, Value(maxDouble)).getFloat());
  ...
  EXPECT_EQ(5.1 + 0.0000000000000001, OptimizerUtils::boundValue(col, OP::GREATER_THAN, Value(5.1)).getFloat());
...
TEST(IndexBoundValueTest, DateTest) {
  ...
  EXPECT_EQ(Date(2021, 1, 1), OptimizerUtils::boundValue(col, OP::GREATER_THAN, Value(Date(2020, 12, 31))));
  EXPECT_EQ(Date(maxYear, 12, 31), OptimizerUtils::boundValue(col, OP::GREATER_THAN, Value(Date(maxYear, 12, 31))));
  EXPECT_EQ(Date(2020, 1, 2), OptimizerUtils::boundValue(col, OP::GREATER_THAN, Value(Date(2020, 1, 1))));
  ...
  EXPECT_EQ(Date(2019, 12, 30), OptimizerUtils::boundValue(col, OP::LESS_THAN, Value(Date(2019, 12, 31))));
  EXPECT_EQ(Date(2018, 12, 31), OptimizerUtils::boundValue(col, OP::LESS_THAN, Value(Date(2019, 1, 1))));
}
...
TEST(IndexBoundValueTest, DateTimeTest) {
...

  {
    ...
    expect.month = 1;
    expect.year = 2021;
    EXPECT_EQ(expect, OptimizerUtils::boundValue(col, OP::GREATER_THAN, Value(actual)).getDateTime());
  }
  {
    DateTime actual, expect;
    actual.microsec = std::numeric_limits<int32_t>::max();
