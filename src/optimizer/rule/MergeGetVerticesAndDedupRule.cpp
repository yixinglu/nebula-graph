...
MergeGetVerticesAndDedupRule::MergeGetVerticesAndDedupRule() { RuleSet::QueryRules().addRule(this); }

const Pattern &MergeGetVerticesAndDedupRule::pattern() const {
  static Pattern pattern =
      Pattern::create(graph::PlanNode::Kind::kGetVertices, {Pattern::create(graph::PlanNode::Kind::kDedup)});
  return pattern;
}

StatusOr<OptRule::TransformResult> MergeGetVerticesAndDedupRule::transform(OptContext *ctx,
...
std::string MergeGetVerticesAndDedupRule::toString() const { return "MergeGetVerticesAndDedupRule"; }
