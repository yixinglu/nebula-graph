...
std::unique_ptr<OptRule> IndexScanRule::kInstance = std::unique_ptr<IndexScanRule>(new IndexScanRule());
...
StatusOr<OptRule::TransformResult> IndexScanRule::transform(OptContext* ctx, const MatchedResult& matched) const {
...
Status IndexScanRule::createIndexQueryCtx(IndexQueryCtx& iqctx, ScanKind kind, const FilterItems& items,
                                          graph::QueryContext* qctx, const OptGroupNode* groupNode) const {
  return kind.isSingleScan() ? createSingleIQC(iqctx, items, qctx, groupNode)
                             : createMultipleIQC(iqctx, items, qctx, groupNode);
}

Status IndexScanRule::createIndexQueryCtx(IndexQueryCtx& iqctx, graph::QueryContext* qctx,
...
Status IndexScanRule::createSingleIQC(IndexQueryCtx& iqctx, const FilterItems& items, graph::QueryContext* qctx,
                                      const OptGroupNode* groupNode) const {
...
Status IndexScanRule::createMultipleIQC(IndexQueryCtx& iqctx, const FilterItems& items, graph::QueryContext* qctx,
                                        const OptGroupNode* groupNode) const {
...
Status IndexScanRule::appendIQCtx(const IndexItem& index, const FilterItems& items, IndexQueryCtx& iqctx,
  ...
  for (const auto& field : fields) {
    ...
    auto it = std::find_if(filterItems.items.begin(), filterItems.items.end(),
                           [](const auto& ite) { return ite.relOP_ == RelationalExpression::Kind::kRelNE; });
...
Status IndexScanRule::appendColHint(std::vector<IndexColumnHint>& hints, const FilterItems& items,
...
Status IndexScanRule::boundValue(const FilterItem& item, const meta::cpp2::ColumnDef& col, Value& begin,
                                 Value& end) const {
...
std::unique_ptr<Expression> IndexScanRule::filterExpr(const OptGroupNode* groupNode) const {
  auto in = static_cast<const IndexScan*>(groupNode->node());
...
Status IndexScanRule::analyzeExpression(Expression* expr, FilterItems* items, ScanKind* kind, bool isEdge) const {
  ...
  switch (expr->kind()) {
    ...
    case Expression::Kind::kLogicalAnd: {
      ...
      auto k =
          expr->kind() == Expression::Kind::kLogicalAnd ? ScanKind::Kind::kSingleScan : ScanKind::Kind::kMultipleScan;
      if (kind->getKind() == ScanKind::Kind::kUnknown) {
        kind->setKind(k);
      } else if (kind->getKind() != k) {
        auto errorMsg = folly::StringPiece("Condition not support yet : %s", expr->toString().c_str());
    ...
    case Expression::Kind::kRelNE: {
      ...
      NG_RETURN_IF_ERROR(ret);
      if (kind->getKind() == ScanKind::Kind::kMultipleScan && expr->kind() == Expression::Kind::kRelNE) {
    ...
    default: {
      auto errorMsg = folly::StringPiece("Filter not support yet : %s", expr->toString().c_str());
      return Status::NotSupported(errorMsg);
    }
  }
  return Status::OK();
...
Status IndexScanRule::addFilterItem(RelationalExpression* expr, FilterItems* items) const {
  ...
  auto relType =
      std::is_same<E, EdgePropertyExpression>::value ? Expression::Kind::kEdgeProperty : Expression::Kind::kTagProperty;
  ...
  } else if (expr->left()->kind() == Expression::Kind::kConstant && expr->right()->kind() == relType) {
...
IndexItem IndexScanRule::findOptimalIndex(graph::QueryContext* qctx, const OptGroupNode* groupNode,
...
IndexItem IndexScanRule::findLightestIndex(graph::QueryContext* qctx, const OptGroupNode* groupNode) const {
...
std::vector<IndexItem> IndexScanRule::allIndexesBySchema(graph::QueryContext* qctx,
  ...
  for (auto& index : ret.value()) {
    ...
    auto id = isEdge(groupNode) ? index->get_schema_id().get_edge_type() : index->get_schema_id().get_tag_id();
...
std::vector<IndexItem> IndexScanRule::findValidIndex(graph::QueryContext* qctx, const OptGroupNode* groupNode,
                                                     const FilterItems& items) const {
  auto indexes = allIndexesBySchema(qctx, groupNode);
  if (indexes.empty()) {
    return indexes;
  ...
  for (const auto& index : indexes) {
    ...
    for (const auto& item : items.items) {
      auto it = std::find_if(fields.begin(), fields.end(),
                             [item](const auto& field) { return field.get_name() == item.col_; });
      if (it == fields.end()) {
        allColsHint = false;
        break;
      }
  ...
  if (!validIndexes.empty()) {
    ...
    while (index != validIndexes.end()) {
      ...
      auto it = std::find_if(items.items.begin(), items.items.end(),
                             [fields](const auto& item) { return item.col_ == fields[0].get_name(); });
...
std::vector<IndexItem> IndexScanRule::findIndexForEqualScan(const std::vector<IndexItem>& indexes,
  ...
  for (auto& index : indexes) {
    ...
    for (const auto& field : index->get_fields()) {
      auto it = std::find_if(items.items.begin(), items.items.end(),
                             [field](const auto& item) { return item.col_ == field.get_name(); });
      if (it == items.items.end()) {
        break;
      }
...
std::vector<IndexItem> IndexScanRule::findIndexForRangeScan(const std::vector<IndexItem>& indexes,
  ...
  for (const auto& index : indexes) {
    ...
    for (const auto& field : index->get_fields()) {
      ...
      if ((*fi).relOP_ == RelationalExpression::Kind::kRelGE || (*fi).relOP_ == RelationalExpression::Kind::kRelGT ||
          (*fi).relOP_ == RelationalExpression::Kind::kRelLE || (*fi).relOP_ == RelationalExpression::Kind::kRelLT) {
        hintCount++;
      } else {
        break;
      }
