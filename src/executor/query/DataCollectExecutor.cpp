...
Status DataCollectExecutor::collectMToN(const std::vector<std::string>& vars, StepClause::MToN* mToN, bool distinct) {
  DataSet ds;
  ds.colNames = std::move(colNames_);
  DCHECK(!ds.colNames.empty());
...
Status DataCollectExecutor::collectMultiplePairShortestPath(const std::vector<std::string>& vars) {
  ...
  // src : {dst : <cost, {path}>}
  std::unordered_map<Value, std::unordered_map<Value, std::pair<Value, std::vector<Path>>>> shortestPath;
  ...
  for (auto& var : vars) {
    ...
    for (auto& result : hist) {
      ...
      for (; seqIter->valid(); seqIter->next()) {
        ...
        if (shortestPath.find(src) == shortestPath.end() || shortestPath[src].find(dst) == shortestPath[src].end()) {
