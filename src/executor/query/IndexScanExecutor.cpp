...
folly::Future<Status> IndexScanExecutor::indexScan() {
  ...
  return storageClient
      ->lookupIndex(lookup->space(), *lookup->queryContext(), lookup->isEdge(), lookup->schemaId(),
                    *lookup->returnColumns())
      .via(runner())
      .thenValue([this](StorageRpcResponse<LookupIndexResp> &&rpcResp) { return handleResp(std::move(rpcResp)); });
}
...
Status IndexScanExecutor::handleResp(storage::StorageRpcResponse<Resp> &&rpcResp) {
  ...
  return finish(ResultBuilder().value(std::move(v)).iter(Iterator::Kind::kProp).state(state).finish());
}
