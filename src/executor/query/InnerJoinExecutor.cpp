...
DataSet InnerJoinExecutor::probe(const std::vector<Expression*>& probeKeys, Iterator* probeIter,
...
DataSet InnerJoinExecutor::singleKeyProbe(Expression* probeKey, Iterator* probeIter,
...
template <class T>
void InnerJoinExecutor::buildNewRow(const std::unordered_map<T, std::vector<const Row*>>& hashTable, const T& val,
                                    const Row& rRow, DataSet& ds) const {
  ...
  for (auto* row : range->second) {
    ...
    if (exchange_) {
      values.insert(values.end(), std::make_move_iterator(rRow.values.begin()),
