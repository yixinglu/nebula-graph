...
folly::Future<Status> UnionExecutor::execute() {
  ...
  ds.rows.insert(ds.rows.end(), std::make_move_iterator(leftIter->begin()), std::make_move_iterator(leftIter->end()));

  DCHECK(right->isSequentialIter());
  auto rightIter = static_cast<SequentialIter*>(right.get());
  ds.rows.insert(ds.rows.end(), std::make_move_iterator(rightIter->begin()), std::make_move_iterator(rightIter->end()));
