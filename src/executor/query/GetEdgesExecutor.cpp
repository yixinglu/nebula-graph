...
folly::Future<Status> GetEdgesExecutor::getEdges() {
  ...
  if (ge->src() != nullptr && ge->type() != nullptr && ge->ranking() != nullptr && ge->dst() != nullptr) {
    ...
    for (; valueIter->valid(); valueIter->next()) {
      ...
      if (!SchemaUtil::isValidVid(src, *spaceInfo.spaceDesc.vid_type_ref()) ||
          !SchemaUtil::isValidVid(dst, *spaceInfo.spaceDesc.vid_type_ref()) || !type.isInt() || !ranking.isInt()) {
        LOG(WARNING) << "Mismatched edge key type";
        continue;
      }
      edges.emplace_back(Row({std::move(src), type, ranking, std::move(dst)}));
    }
  }

  if (edges.rows.empty()) {
    // TODO: add test for empty input.
    return finish(ResultBuilder().value(Value(DataSet(ge->colNames()))).iter(Iterator::Kind::kProp).finish());
  }

  time::Duration getPropsTime;
  return DCHECK_NOTNULL(client)
      ->getProps(ge->space(), std::move(edges), nullptr, &ge->props(), ge->exprs().empty() ? nullptr : &ge->exprs(),
                 ge->dedup(), ge->orderBy(), ge->limit(), ge->filter())
      .via(runner())
      .ensure([this, getPropsTime]() {
        SCOPED_TIMER(&execTime_);
        otherStats_.emplace("total_rpc", folly::stringPrintf("%lu(us)", getPropsTime.elapsedInUSec()));
      })
