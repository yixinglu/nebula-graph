...
Status SetExecutor::checkInputDataSets() {
  ...
  if (UNLIKELY(lIter->kind() == Iterator::Kind::kGetNeighbors || rIter->kind() == Iterator::Kind::kGetNeighbors)) {
    std::stringstream ss;
  ...
  if (UNLIKELY(!leftData || !rightData)) {
    return Status::Error("SET related executor failed, %s side input dataset is null", !leftData ? "left" : "right");
  }
  ...
  return Status::Error("Datasets have different columns: <%s> vs. <%s>", lcols.c_str(), rcols.c_str());
}
