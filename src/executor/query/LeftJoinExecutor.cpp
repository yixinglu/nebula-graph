...
DataSet LeftJoinExecutor::probe(const std::vector<Expression*>& probeKeys, Iterator* probeIter,
...
DataSet LeftJoinExecutor::singleKeyProbe(Expression* probeKey, Iterator* probeIter,
...
template <class T>
void LeftJoinExecutor::buildNewRow(const std::unordered_map<T, std::vector<const Row*>>& hashTable, const T& val,
                                   const Row& lRow, DataSet& ds) const {
  ...
  if (range == hashTable.end()) {
    ...
    values.insert(values.end(), std::make_move_iterator(lRow.values.begin()),
