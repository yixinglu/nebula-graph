...
folly::Future<Status> CreateEdgeIndexExecutor::execute() {
  ...
  return qctx()
      ->getMetaClient()
      ->createEdgeIndex(spaceId, ceiNode->getIndexName(), ceiNode->getSchemaName(), ceiNode->getFields(),
                        ceiNode->getIfNotExists())
      .via(runner())
      .thenValue([ceiNode, spaceId](StatusOr<IndexID> resp) {
        if (!resp.ok()) {
          LOG(ERROR) << "SpaceId: " << spaceId << ", Create index `" << ceiNode->getIndexName() << "' at edge: `"
                     << ceiNode->getSchemaName() << "' failed: " << resp.status();
          return resp.status();
        }
        return Status::OK();
      });
...
folly::Future<Status> DescEdgeIndexExecutor::execute() {
  ...
  return qctx()
      ...
      .thenValue([this, deiNode, spaceId](StatusOr<meta::cpp2::IndexItem> resp) {
        ...
        return finish(ResultBuilder().value(std::move(ret).value()).iter(Iterator::Kind::kDefault).finish());
      });
}
...
folly::Future<Status> ShowCreateEdgeIndexExecutor::execute() {
  ...
  return qctx()
      ...
      .thenValue([this, sceiNode, spaceId](StatusOr<meta::cpp2::IndexItem> resp) {
        if (!resp.ok()) {
          LOG(ERROR) << "SpaceId: " << spaceId << ", Show create edge index `" << sceiNode->getIndexName()
                     << "' failed: " << resp.status();
          return resp.status();
        }
...
folly::Future<Status> ShowEdgeIndexStatusExecutor::execute() {
  ...
  return qctx()->getMetaClient()->listEdgeIndexStatus(spaceId).via(runner()).thenValue(
      [this, spaceId](StatusOr<std::vector<meta::cpp2::IndexStatus>> resp) {
        if (!resp.ok()) {
          LOG(ERROR) << "SpaceId: " << spaceId << ", Show edge index status failed" << resp.status();
          return resp.status();
        }
