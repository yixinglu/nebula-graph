...
folly::Future<Status> DescEdgeExecutor::execute() {
  ...
  return qctx()
      ...
      .thenValue([this, deNode, spaceId](StatusOr<meta::cpp2::Schema> resp) {
        ...
        if (!ret.ok()) {
          LOG(ERROR) << "SpaceId: " << spaceId << ", Desc edge `" << deNode->getName() << "' failed: " << resp.status();
          return ret.status();
        }
        return finish(ResultBuilder().value(Value(std::move(ret).value())).iter(Iterator::Kind::kDefault).finish());
      });
}
...
folly::Future<Status> DropEdgeExecutor::execute() {
  ...
  return qctx()
      ...
      .thenValue([deNode, spaceId](StatusOr<bool> resp) {
        if (!resp.ok()) {
          LOG(ERROR) << "SpaceId: " << spaceId << ", Drop edge `" << deNode->getName() << "' failed: " << resp.status();
          return resp.status();
        }
        return Status::OK();
      });
...
folly::Future<Status> AlterEdgeExecutor::execute() {
  ...
  return qctx()
      ->getMetaClient()
      ->alterEdgeSchema(aeNode->space(), aeNode->getName(), aeNode->getSchemaItems(), aeNode->getSchemaProp())
      .via(runner())
