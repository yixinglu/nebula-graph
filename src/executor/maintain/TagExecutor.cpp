...
folly::Future<Status> DescTagExecutor::execute() {
  ...
  return qctx()
      ...
      .thenValue([this, dtNode, spaceId](StatusOr<meta::cpp2::Schema> resp) {
        if (!resp.ok()) {
          LOG(ERROR) << "SpaceId: " << spaceId << ", Desc tag `" << dtNode->getName() << "' failed: " << resp.status();
          return resp.status();
        }
...
folly::Future<Status> DropTagExecutor::execute() {
  ...
  return qctx()
      ...
      .thenValue([dtNode, spaceId](StatusOr<bool> resp) {
        if (!resp.ok()) {
          LOG(ERROR) << "SpaceId: " << spaceId << ", Drop tag `" << dtNode->getName() << "' failed: " << resp.status();
          return resp.status();
        }
        return Status::OK();
      });
...
folly::Future<Status> AlterTagExecutor::execute() {
  ...
  return qctx()
      ->getMetaClient()
      ->alterTagSchema(aeNode->space(), aeNode->getName(), aeNode->getSchemaItems(), aeNode->getSchemaProp())
      .via(runner())
