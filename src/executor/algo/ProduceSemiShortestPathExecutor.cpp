...
std::vector<Path> ProduceSemiShortestPathExecutor::createPaths(const std::vector<const Path*>& paths,
                                                               const Edge& edge) {
...
void ProduceSemiShortestPathExecutor::dstInCurrent(const Edge& edge, CostPathMapType& currentCostPathMap) {
  auto& src = edge.src;
  auto& dst = edge.dst;
...
void ProduceSemiShortestPathExecutor::dstNotInHistory(const Edge& edge, CostPathMapType& currentCostPathMap) {
  auto& src = edge.src;
  auto& dst = edge.dst;
  auto weight = 1;  // weight = weight_->getWeight();
  auto& srcPaths = historyCostPathMap_[src];
  if (currentCostPathMap.find(dst) == currentCostPathMap.end()) {
    ...
    for (auto& srcPath : srcPaths) {
      ...
      std::vector<Path> newPaths = createPaths(srcPath.second.paths_, edge);
      std::unordered_map<Value, CostPaths> temp = {{srcPath.first, CostPaths(cost, newPaths)}};
...
void ProduceSemiShortestPathExecutor::dstInHistory(const Edge& edge, CostPathMapType& currentCostPathMap) {
  auto& src = edge.src;
  auto& dst = edge.dst;
  auto weight = 1;  // weight = weight_->getWeight();
  auto& srcPaths = historyCostPathMap_[src];
  ...
  if (currentCostPathMap.find(dst) == currentCostPathMap.end()) {
    ...
    for (auto& srcPath : srcPaths) {
      if (historyCostPathMap_[dst].find(srcPath.first) == historyCostPathMap_[dst].end()) {
        ...
        auto newCost = srcPath.second.cost_ + weight;
        std::vector<Path> newPaths = createPaths(srcPath.second.paths_, edge);
        std::unordered_map<Value, CostPaths> temp = {{srcPath.first, CostPaths(newCost, newPaths)}};
        currentCostPathMap.emplace(dst, std::move(temp));
      } else {
...
void ProduceSemiShortestPathExecutor::updateHistory(const Value& dst, const Value& src, double cost, Value& paths) {
