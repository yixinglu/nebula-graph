...
folly::Future<Status> ConjunctPathExecutor::bfsShortestPath() {
  ...
  VLOG(1) << "left input: " << conjunct->leftInputVar() << " right input: " << conjunct->rightInputVar();
  DCHECK(!!lIter);
...
std::vector<Row> ConjunctPathExecutor::findBfsShortestPath(Iterator* iter, bool isLatest,
...
std::unordered_multimap<Value, Path> ConjunctPathExecutor::buildBfsInterimPath(
    std::unordered_set<Value>& meets, std::vector<std::unordered_multimap<Value, const Edge*>>& hists) {
...
Status ConjunctPathExecutor::conjunctPath(const List& forwardPaths, const List& backwardPaths, Value& cost,
                                          DataSet& ds) {
...
bool ConjunctPathExecutor::findPath(Iterator* backwardPathIter, CostPathsValMap& forwardPathTable, DataSet& ds) {
  bool found = false;
...
bool ConjunctPathExecutor::findAllPaths(Iterator* backwardPathsIter,
                                        std::unordered_map<Value, const List&>& forwardPathsTable, DataSet& ds) {
  bool found = false;
