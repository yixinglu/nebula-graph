...
class ConjunctPathExecutor final : public Executor {
 public:
  ConjunctPathExecutor(const PlanNode* node, QueryContext* qctx) : Executor("ConjunctPathExecutor", node, qctx) {}
 ...
 private:
  ...
  std::vector<Row> findBfsShortestPath(Iterator* iter, bool isLatest,
                                       std::unordered_multimap<Value, const Edge*>& table);

  std::unordered_multimap<Value, Path> buildBfsInterimPath(
      std::unordered_set<Value>& meets, std::vector<std::unordered_multimap<Value, const Edge*>>& hist);
  ...
  bool findAllPaths(Iterator* backwardPathsIter, std::unordered_map<Value, const List&>& forwardPathsTable,
                    DataSet& ds);
