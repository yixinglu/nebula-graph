  ...
  return qctx()
      ...
      .thenValue([this, scNode](StatusOr<std::vector<meta::cpp2::ConfigItem>> resp) {
        if (!resp.ok()) {
          ...
          return Status::Error("Show config `%s' failed: %s", module.c_str(), resp.status().toString().c_str());
        }
  ...
  return qctx()
      ...
      .thenValue([scNode](StatusOr<bool> resp) {
        if (!resp.ok()) {
          ...
          return Status::Error("Set config `%s' failed: %s", scNode->getName().c_str(),
  ...
  return qctx()
      ...
      .thenValue([this, gcNode](StatusOr<std::vector<meta::cpp2::ConfigItem>> resp) {
        if (!resp.ok()) {
          ...
          return Status::Error("Get config `%s' failed: %s", gcNode->getName().c_str(),
                               resp.status().toString().c_str());
        }
        auto result = generateResult(resp.value());
        return finish(ResultBuilder().value(Value(std::move(result))).finish());
