  ...
  return qctx()
      ...
      .thenValue([this](StatusOr<std::vector<meta::cpp2::BalanceTask>> resp) {
        ...
        for (auto &task : tasks) {
          ...
          v.emplace_back(Row({std::move(task).get_id(), apache::thrift::util::enumNameSafe(task.get_result())}));
        }
        double percentage = total == 0 ? 0 : static_cast<double>(succeeded) / total * 100;
        v.emplace_back(Row({folly::sformat("Total:{}, Succeeded:{}, Failed:{}, In Progress:{}, Invalid:{}", total,
                                           succeeded, failed, inProgress, invalid),
                            percentage}));
