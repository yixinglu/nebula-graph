...
folly::Future<Status> GrantRoleExecutor::grantRole() {
  ...
  NG_RETURN_IF_ERROR(PermissionManager::canWriteRole(session, grNode->role(), spaceId, *grNode->username()));
  ...
  return qctx()->getMetaClient()->grantToUser(std::move(item)).via(runner()).thenValue([this](StatusOr<bool> resp) {
    SCOPED_TIMER(&execTime_);
