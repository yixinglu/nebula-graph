...
folly::Future<Status> DescSpaceExecutor::execute() {
  ...
  return qctx()
      ...
      .thenValue([this](StatusOr<meta::cpp2::SpaceItem> resp) {
        ...
        DataSet dataSet;
        dataSet.colNames = {"ID",      "Name",     "Partition Number", "Replica Factor", "Charset",
                            "Collate", "Vid Type", "Atomic Edge",      "Group"};
        Row row;
        ...
        return finish(ResultBuilder().value(Value(std::move(dataSet))).iter(Iterator::Kind::kDefault).finish());
      });
}
...
folly::Future<Status> ShowCreateSpaceExecutor::execute() {
  ...
  return qctx()
      ...
      .thenValue([this, scsNode](StatusOr<meta::cpp2::SpaceItem> resp) {
        if (!resp.ok()) {
          LOG(ERROR) << "Show create space `" << scsNode->getSpaceName() << "' failed: " << resp.status();
          return resp.status();
        }
        ...
        row.values.emplace_back(folly::stringPrintf(
            fmt, properties.get_space_name().c_str(), properties.get_partition_num(), properties.get_replica_factor(),
            properties.get_charset_name().c_str(), properties.get_collate_name().c_str(),
            SchemaUtil::typeToString(properties.get_vid_type()).c_str(), sAtomicEdge.c_str(),
            properties.group_name_ref().has_value() ? (*properties.group_name_ref()).c_str() : "default"));
        dataSet.rows.emplace_back(std::move(row));
        return finish(ResultBuilder().value(Value(std::move(dataSet))).iter(Iterator::Kind::kDefault).finish());
      });
}
