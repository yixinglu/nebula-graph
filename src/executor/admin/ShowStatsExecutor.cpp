...
folly::Future<Status> ShowStatsExecutor::execute() {
  ...
  return qctx()->getMetaClient()->getStatis(spaceId).via(runner()).thenValue(
      [this, spaceId](StatusOr<meta::cpp2::StatisItem> resp) {
        ...
        std::sort(tagCount.begin(), tagCount.end(), [](const auto& l, const auto& r) { return l.first < r.first; });
        std::sort(edgeCount.begin(), edgeCount.end(), [](const auto& l, const auto& r) { return l.first < r.first; });
