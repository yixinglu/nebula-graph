...
folly::Future<Status> ShowHostsExecutor::showHosts() {
  ...
  auto makeTranditionalResult = [&](const std::vector<meta::cpp2::HostItem> &hostVec) -> DataSet {
    DataSet v({"Host", "Port", "Status", "Leader count", "Leader distribution", "Partition distribution"});
    ...
    for (const auto &host : hostVec) {
      nebula::Row r(
          {host.get_hostAddr().host, host.get_hostAddr().port, apache::thrift::util::enumNameSafe(host.get_status())});
      ...
      std::map<std::string, std::vector<PartitionID>> lPartsCount(leaderParts.begin(), leaderParts.end());
  ...
  auto makeGitInfoResult = [&](const std::vector<meta::cpp2::HostItem> &hostVec) -> DataSet {
    ...
    for (const auto &host : hostVec) {
      nebula::Row r({host.get_hostAddr().host, host.get_hostAddr().port,
                     apache::thrift::util::enumNameSafe(host.get_status()),
                     apache::thrift::util::enumNameSafe(host.get_role()), host.get_git_info_sha()});
      v.emplace_back(std::move(r));
    }  // row loop
    return v;
  };
