...
#define TEST_AGG_2(FUN, COL, DISTINCT)                                                            \
  std::vector<Expression*> groupKeys;                                                             \
  std::vector<Expression*> groupItems;                                                            \
  auto expr1 = std::make_unique<InputPropertyExpression>(new std::string("col2"));                \
...
#define TEST_AGG_3(FUN, COL, DISTINCT)                                                            \
  ...
  groupItems.emplace_back(&item);                                                                 \
  auto expr3 = std::make_unique<InputPropertyExpression>(new std::string("col3"));                \
...
TEST_F(AggregateTest, Collect) {
  ...
  }
  {
    ...
    std::vector<Expression*> groupKeys;
    std::vector<Expression*> groupItems;
    auto expr1 = std::make_unique<InputPropertyExpression>(new std::string("col1"));
    auto expr2 = expr1->clone();
    groupKeys.emplace_back(expr1.get());
