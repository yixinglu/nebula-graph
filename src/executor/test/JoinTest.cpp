...
void JoinTest::testInnerJoin(std::string left, std::string right, DataSet& expected, int64_t line) {
  VariablePropertyExpression key(new std::string(left), new std::string("dst"));
  std::vector<Expression*> hashKeys = {&key};
  VariablePropertyExpression probe(new std::string(right), new std::string("_vid"));
  std::vector<Expression*> probeKeys = {&probe};

  auto* join = InnerJoin::make(qctx_.get(), nullptr, {left, 0}, {right, 0}, std::move(hashKeys), std::move(probeKeys));
...
void JoinTest::testLeftJoin(std::string left, std::string right, DataSet& expected, int64_t line) {
  ...
  auto* join = LeftJoin::make(qctx_.get(), nullptr, {left, 0}, {right, 0}, std::move(hashKeys), std::move(probeKeys));
...
TEST_F(JoinTest, InnerJoinTwice) {
  std::string joinOutput;
  {
    ...
    auto* join =
        InnerJoin::make(qctx_.get(), nullptr, {left, 0}, {right, 0}, std::move(hashKeys), std::move(probeKeys));
    join->setColNames(std::vector<std::string>{"src", "dst", kVid, "tag_prop", "edge_prop", kDst});
  ...
  auto* join = InnerJoin::make(qctx_.get(), nullptr, {left, 0}, {right, 0}, std::move(hashKeys), std::move(probeKeys));
  join->setColNames(std::vector<std::string>{"src", "dst", kVid, "tag_prop", "edge_prop", kDst, "col1"});
...
TEST_F(JoinTest, LeftJoinTwice) {
  ...
  auto* join = LeftJoin::make(qctx_.get(), nullptr, {left, 0}, {right, 0}, std::move(hashKeys), std::move(probeKeys));
  join->setColNames(std::vector<std::string>{kVid, "tag_prop", "edge_prop", kDst, "src", "dst", "col1"});
