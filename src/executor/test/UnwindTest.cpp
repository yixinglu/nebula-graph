...
TEST_F(UnwindTest, UnwindNestedList) {
  ...
  DataSet expected;
  expected.colNames = {"r"};
  expected.rows = {Row({Value(1)}), Row({Value(List({Value(2), Value::kNullValue, Value(3)}))}), Row({Value(4)}),
...
TEST_F(UnwindTest, UnwindLabel) {
  ...
  auto col2 = new YieldColumn(new VariablePropertyExpression(new std::string(), new std::string("r1")));
  ...
  expected.rows = {Row({Value(1)}), Row({Value(2)}), Row({Value::kNullValue}), Row({Value(3)}), Row({Value(4)})};
  EXPECT_EQ(result.value().getDataSet(), expected);
  EXPECT_EQ(result.state(), Result::State::kSuccess);
}
...
TEST_F(UnwindTest, ProjectUnwind) {
  ...
  */
  ...
  auto col = new YieldColumn(new VariablePropertyExpression(new std::string(), new std::string("b")));
