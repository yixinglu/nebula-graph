...
class QueryTestBase : public testing::Test {
 protected:
  void SetUp() override {
    ...
    // GetNeighbors
    {
      DataSet dataset({kVid, "_stats", "_tag:person:name:age", "_edge:+study:_dst:start_year:end_year", "_expr"});
      {
        Row row;
        // vid
      ...
      auto result = ResultBuilder().value(Value(datasetList)).iter(Iterator::Kind::kGetNeighbors).finish();
    ...
    // sequential init by two sequentialIters
    {
      ...
      builder.value(lIter->valuePtr()).iter(std::make_unique<SequentialIter>(std::move(lIter), std::move(rIter)));
