...
class DataCollectTest : public testing::Test {
 protected:
  void SetUp() override {
    ...
    {
      DataSet ds1;
      ds1.colNames = {kVid, "_stats", "_tag:tag1:prop1:prop2", "_edge:+edge1:prop1:prop2:_dst:_rank", "_expr"};
      for (auto i = 0; i < 10; ++i) {
        Row row;
        // _vid
      ...
      DataSet ds2;
      ds2.colNames = {kVid, "_stats", "_tag:tag2:prop1:prop2", "_edge:-edge2:prop1:prop2:_dst:_rank", "_expr"};
      ...
      qctx_->ectx()->setResult(
          "input_datasets", ResultBuilder().value(Value(std::move(ds2))).iter(Iterator::Kind::kGetNeighbors).finish());
    }
    {
      DataSet ds;
      ...
      qctx_->ectx()->setResult("input_sequential", ResultBuilder().value(Value(std::move(ds))).finish());
    }
    {
      DataSet ds;
      ds.colNames = {kVid, "_stats", "_tag:tag1:prop1:prop2", "_edge:+edge1:prop1:prop2:_dst:_rank", "_expr"};
...
TEST_F(DataCollectTest, CollectSubgraph) {
  auto* dc = DataCollect::make(qctx_.get(), nullptr, DataCollect::CollectKind::kSubgraph, {"input_datasets"});
  ...
  for (auto& e : originEdges.values) {
    ...
    auto edgeKey = std::make_tuple(e.getEdge().src, e.getEdge().type, e.getEdge().ranking, e.getEdge().dst);
...
TEST_F(DataCollectTest, RowBasedMove) {
  auto* dc = DataCollect::make(qctx_.get(), nullptr, DataCollect::CollectKind::kRowBasedMove, {"input_sequential"});
...
TEST_F(DataCollectTest, EmptyResult) {
  auto* dc = DataCollect::make(qctx_.get(), nullptr, DataCollect::CollectKind::kSubgraph, {"empty_get_neighbors"});
  dc->setColNames(std::vector<std::string>{"_vertices", "_edges"});
