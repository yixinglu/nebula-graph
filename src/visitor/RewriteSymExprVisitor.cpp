...
RewriteSymExprVisitor::RewriteSymExprVisitor(const std::string &sym, bool isEdge) : sym_(sym), isEdge_(isEdge) {}
...
void RewriteSymExprVisitor::visit(LabelExpression *expr) {
  if (isEdge_) {
    expr_ = std::make_unique<EdgePropertyExpression>(new std::string(sym_), new std::string(*expr->name()));
  } else {
    expr_ = std::make_unique<SourcePropertyExpression>(new std::string(sym_), new std::string(*expr->name()));
  }
}
