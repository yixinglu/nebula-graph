...
/*static*/ VidExtractVisitor::VidPattern VidExtractVisitor::intersect(VidPattern &&left, VidPattern &&right) {
  ...
  for (auto &node : right.nodes) {
    ...
    if (find == v.nodes.end()) {
      ...
      std::set_intersection(find->second.vids.values.begin(), find->second.vids.values.end(),
                            node.second.vids.values.begin(), node.second.vids.values.end(),
...
/*static*/ VidExtractVisitor::VidPattern VidExtractVisitor::intersect(
  ...
  if (find == left.nodes.end()) {
    ...
    std::set_intersection(find->second.vids.values.begin(), find->second.vids.values.end(),
                          right.second.vids.values.begin(), right.second.vids.values.end(), std::back_inserter(values));
...
/*static*/ VidExtractVisitor::VidPattern VidExtractVisitor::intersect(
    std::pair<std::string, VidPattern::Vids> &&left, std::pair<std::string, VidPattern::Vids> &&right) {
  ...
  if (left.first != right.first) {
    ...
    std::sort(right.second.vids.values.begin(), right.second.vids.values.end());
    std::vector<Value> values;
    std::set_intersection(left.second.vids.values.begin(), left.second.vids.values.end(),
                          right.second.vids.values.begin(), right.second.vids.values.end(), std::back_inserter(values));
...
void VidExtractVisitor::visit(RelationalExpression *expr) {
  if (expr->kind() == Expression::Kind::kRelIn) {
    ...
    if (expr->left()->kind() == Expression::Kind::kLabelAttribute) {
      ...
      vidPattern_ = VidPattern{VidPattern::Special::kInUsed, {{label, {VidPattern::Vids::Kind::kOtherSource, {}}}}};
      return;
    }
    ...
    vidPattern_ = VidPattern{
        ...
        {{fCallExpr->args()->args().front()->toString(), {VidPattern::Vids::Kind::kIn, constExpr->value().getList()}}}};
    return;
    ...
    vidPattern_ = VidPattern{
        VidPattern::Special::kInUsed,
        {{fCallExpr->args()->args().front()->toString(), {VidPattern::Vids::Kind::kIn, List({constExpr->value()})}}}};
    return;
  } else {
    vidPattern_ = VidPattern{};
    return;
      ...
      if (operandResult.spec == VidPattern::Special::kInUsed) {
        for (auto &node : operandResult.nodes) {
          if (node.second.kind == VidPattern::Vids::Kind::kNotIn) {
            notInResult.nodes[node.first].vids.values.insert(notInResult.nodes[node.first].vids.values.end(),
        ...
        for (auto &node : result.nodes) {
          ...
          switch (node.second.kind) {
            ...
            case VidPattern::Vids::Kind::kIn: {
              ...
              inResult.nodes[node.first].vids.values.insert(inResult.nodes[node.first].vids.values.end(),
                                                            std::make_move_iterator(node.second.vids.values.begin()),
                                                            std::make_move_iterator(node.second.vids.values.end()));
            }
            case VidPattern::Vids::Kind::kNotIn:
