...
void ExpressionProps::insertTagNameIds(const std::string &name, TagID tagId) { tagNameIds_.emplace(name, tagId); }
...
// visitor
DeducePropsVisitor::DeducePropsVisitor(QueryContext *qctx, GraphSpaceID space, ExpressionProps *exprProps,
                                       std::set<std::string> *userDefinedVarNameList)
    : qctx_(qctx), space_(space), exprProps_(exprProps), userDefinedVarNameList_(userDefinedVarNameList) {
  DCHECK(qctx != nullptr);
...
void DeducePropsVisitor::visit(InputPropertyExpression *expr) { exprProps_->insertInputProp(*expr->prop()); }
