...
ExtractPropExprVisitor::ExtractPropExprVisitor(ValidateContext* vctx, YieldColumns* srcAndEdgePropCols,
                                               YieldColumns* dstPropCols, YieldColumns* inputPropCols,
...
void ExtractPropExprVisitor::visitVertexEdgePropExpr(PropertyExpression* expr) {
  ...
  if (found == propExprColMap_.end()) {
    auto newExpr = propExpr->clone();
    auto col = new YieldColumn(newExpr.release(), new std::string(vctx_->anonColGen()->getCol()));
    propExprColMap_.emplace(propExpr->toString(), col);
...
void ExtractPropExprVisitor::visit(SourcePropertyExpression* expr) { visitVertexEdgePropExpr(expr); }
