...
static const std::unordered_map<Value::Type, Value> kConstantValues = {
    {Value::Type::__EMPTY__, Value()},        {Value::Type::NULLVALUE, Value(NullType::__NULL__)},
    {Value::Type::BOOL, Value(true)},         {Value::Type::INT, Value(1)},
    {Value::Type::FLOAT, Value(1.0)},         {Value::Type::STRING, Value("123")},
    {Value::Type::DATE, Value(Date())},       {Value::Type::DATETIME, Value(DateTime())},
    {Value::Type::VERTEX, Value(Vertex())},   {Value::Type::EDGE, Value(Edge())},
    {Value::Type::PATH, Value(Path())},       {Value::Type::LIST, Value(List())},
    {Value::Type::MAP, Value(Map())},         {Value::Type::SET, Value(Set())},
...
DeduceTypeVisitor::DeduceTypeVisitor(QueryContext *qctx, ValidateContext *vctx, const ColsDef &inputs,
                                     GraphSpaceID space)
...
void DeduceTypeVisitor::visit(TypeCastingExpression *expr) {
  ...
  if (!visitor.ok()) {
    ...
    out << "Can not convert " << expr->operand()->toString() << " 's type : " << type_ << " to " << expr->type();
...
void DeduceTypeVisitor::visit(SubscriptExpression *expr) {
  ...
  if (!ok()) return;
  auto leftType = type_;
  static auto leftCandidate = Value::Type::LIST | Value::Type::MAP | Value::Type::NULLVALUE | Value::Type::__EMPTY__;
  auto isLeftCandidate = leftType & leftCandidate;
  if (!isLeftCandidate) {
    std::stringstream ss;
    ss << "`" << expr->toString() << "', expected LIST but was " << leftType << ": " << expr->left()->toString();
    status_ = Status::SemanticError(ss.str());
    return;
  }
  ...
  static auto rightListSubCandidate = Value::Type::INT | Value::Type::NULLVALUE | Value::Type::__EMPTY__;
  auto notValidListCandidate = (leftListCandidate & leftType) && !(rightListSubCandidate & rightType);
  if (notValidListCandidate) {
    std::stringstream ss;
    ss << "`" << expr->toString() << "', expected Integer but was " << rightType << ": " << expr->right()->toString();
    status_ = Status::SemanticError(ss.str());
    return;
  }

  static auto leftMapCandidate = Value::Type::MAP;
  static auto rightMapSubCandidate = Value::Type::STRING | Value::Type::NULLVALUE | Value::Type::__EMPTY__;
  ...
  static auto rightCandidate = Value::Type::INT | Value::Type::STRING | Value::Type::NULLVALUE | Value::Type::__EMPTY__;
...
void DeduceTypeVisitor::visit(AttributeExpression *expr) {
  ...
  if (type_ != Value::Type::STRING && !isSuperiorType(type_)) {
    std::stringstream ss;
    ss << "`" << expr->toString() << "', expected an valid identifier: " << expr->right()->toString();
    status_ = Status::SemanticError(ss.str());
    return;
  }
...
void DeduceTypeVisitor::visit(LabelAttributeExpression *expr) {
  ...
  if (type_ != Value::Type::STRING && !isSuperiorType(type_)) {
    std::stringstream ss;
    ss << "`" << expr->toString() << "', expected an valid identifier: " << expr->left()->toString();
    status_ = Status::SemanticError(ss.str());
    return;
  }
...
void DeduceTypeVisitor::visit(FunctionCallExpression *expr) {
  ...
  if (!result.ok()) {
    status_ = Status::SemanticError("`%s' is not a valid expression : %s", expr->toString().c_str(),
...
void DeduceTypeVisitor::visit(EdgePropertyExpression *expr) {
  ...
  if (!schema) {
    status_ = Status::SemanticError("`%s', not found edge `%s'.", expr->toString().c_str(), edge->c_str());
    return;
  }
  ...
  if (field == nullptr) {
    status_ = Status::SemanticError("`%s', not found the property `%s'.", expr->toString().c_str(), prop->c_str());
    return;
  }
...
void DeduceTypeVisitor::visit(InputPropertyExpression *expr) {
  auto *prop = expr->prop();
  auto found = std::find_if(inputs_.cbegin(), inputs_.cend(), [&prop](auto &col) { return *prop == col.name; });
  if (found == inputs_.cend()) {
    status_ = Status::SemanticError("`%s', not exist prop `%s'", expr->toString().c_str(), prop->c_str());
    return;
  }
  type_ = found->type;
}
...
void DeduceTypeVisitor::visit(VariablePropertyExpression *expr) {
  ...
  if (!vctx_->existVar(*var)) {
    status_ = Status::SemanticError("`%s', not exist variable `%s'", expr->toString().c_str(), var->c_str());
    return;
  }
  auto *prop = expr->prop();
  auto cols = vctx_->getVar(*var);
  auto found = std::find_if(cols.begin(), cols.end(), [&prop](auto &col) { return *prop == col.name; });
  if (found == cols.end()) {
    status_ = Status::SemanticError("`%s', not exist prop `%s'", expr->toString().c_str(), prop->c_str());
    return;
  }
...
void DeduceTypeVisitor::visitVertexPropertyExpr(PropertyExpression *expr) {
  ...
  if (!schema) {
    status_ = Status::SemanticError("`%s', not found tag `%s'.", expr->toString().c_str(), tag->c_str());
    return;
  }
  auto *prop = expr->prop();
  auto *field = schema->field(*prop);
