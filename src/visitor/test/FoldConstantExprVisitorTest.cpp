...
class FoldConstantExprVisitorTest : public ::testing::Test {
 public:
  void TearDown() override { pool.clear(); }

  static ConstantExpression *constantExpr(Value value) { return new ConstantExpression(std::move(value)); }
  ...
  static TypeCastingExpression *castExpr(Type type, Expression *expr) { return new TypeCastingExpression(type, expr); }

  static UnaryExpression *notExpr(Expression *expr) { return new UnaryExpression(Expression::Kind::kUnaryNot, expr); }
  ...
  static MapExpression *mapExpr(std::initializer_list<std::pair<std::string, Expression *>> exprs) {
  ...
  static SubscriptExpression *subExpr(Expression *lhs, Expression *rhs) { return new SubscriptExpression(lhs, rhs); }

  static FunctionCallExpression *fnExpr(std::string fn, std::initializer_list<Expression *> args) {
  ...
  static VariableExpression *varExpr(const std::string &name) { return new VariableExpression(new std::string(name)); }

  static CaseExpression *caseExpr(Expression *cond, Expression *defaltResult, Expression *when, Expression *then) {
...
TEST_F(FoldConstantExprVisitorTest, TestRelationExpr) {
  ...
  auto expr = pool.add(
      eqExpr(constantExpr(false), notExpr(gtExpr(constantExpr(3), addExpr(constantExpr(1), constantExpr(1))))));
...
TEST_F(FoldConstantExprVisitorTest, TestLogicalExpr) {
  ...
  auto expr = pool.add(
      andExpr(constantExpr(false),
              orExpr(constantExpr(false), gtExpr(constantExpr(3), addExpr(constantExpr(1), constantExpr(1))))));
...
TEST_F(FoldConstantExprVisitorTest, TestSubscriptExpr) {
  ...
  auto expr = pool.add(addExpr(
      constantExpr(1),
      subExpr(listExpr({constantExpr(1), fnExpr("pow", {constantExpr(2), addExpr(constantExpr(2), constantExpr(1))}),
                        constantExpr(2)}),
...
TEST_F(FoldConstantExprVisitorTest, TestListExpr) {
  ...
  auto expr = pool.add(
      listExpr({addExpr(constantExpr(3), constantExpr(4)),
                fnExpr("pow", {constantExpr(2), addExpr(constantExpr(2), constantExpr(1))}), constantExpr(2)}));
...
TEST_F(FoldConstantExprVisitorTest, TestSetExpr) {
  ...
  auto expr =
      pool.add(setExpr({fnExpr("sqrt", {minusExpr(constantExpr(19), constantExpr(3))}),
                        fnExpr("pow", {constantExpr(2), addExpr(constantExpr(3), constantExpr(1))}), constantExpr(2)}));
...
TEST_F(FoldConstantExprVisitorTest, TestMapExpr) {
  ...
  auto expr = pool.add(mapExpr({{"jack", constantExpr(1)},
                                {"tom", fnExpr("pow", {constantExpr(2), addExpr(constantExpr(2), constantExpr(1))})},
                                {"jerry", minusExpr(constantExpr(5), constantExpr(1))}}));
  auto expected = pool.add(mapExpr({{"jack", constantExpr(1)}, {"tom", constantExpr(8)}, {"jerry", constantExpr(4)}}));
  FoldConstantExprVisitor visitor;
  expr->accept(&visitor);
  ASSERT_EQ(*expr, *expected) << expr->toString() << " vs. " << expected->toString();
  ASSERT(visitor.canBeFolded());
...
TEST_F(FoldConstantExprVisitorTest, TestCaseExpr) {
  ...
  auto expr = pool.add(caseExpr(fnExpr("pow", {constantExpr(2), addExpr(constantExpr(2), constantExpr(1))}),
                                addExpr(constantExpr(7), constantExpr(8)), addExpr(constantExpr(2), constantExpr(3)),
                                minusExpr(constantExpr(5), constantExpr(1))));
  auto expected = pool.add(caseExpr(constantExpr(8), constantExpr(15), constantExpr(5), constantExpr(4)));
  FoldConstantExprVisitor visitor;
  expr->accept(&visitor);
  ASSERT_EQ(*expr, *expected) << expr->toString() << " vs. " << expected->toString();
  ASSERT(visitor.canBeFolded());
...
TEST_F(FoldConstantExprVisitorTest, TestFoldFailed) {
  ...
  {
    // pow($v, (1+2)) => pow($v, 3)
    auto expr = pool.add(fnExpr("pow", {varExpr("v"), addExpr(constantExpr(1), constantExpr(2))}));
  ...
  // list
  {
    ...
    auto expr = pool.add(subExpr(listExpr({varExpr("v"), fnExpr("pow", {constantExpr(1), constantExpr(2)}),
                                           addExpr(constantExpr(1), constantExpr(2))}),
                                 minusExpr(constantExpr(1), constantExpr(1))));
    auto expected = pool.add(subExpr(listExpr({varExpr("v"), constantExpr(1), constantExpr(3)}), constantExpr(0)));
    FoldConstantExprVisitor visitor;
    expr->accept(&visitor);
    ASSERT_EQ(*expr, *expected) << expr->toString() << " vs. " << expected->toString();
    ASSERT_FALSE(visitor.canBeFolded());
