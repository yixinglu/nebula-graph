...
void ExecutionContext::deleteValue(const std::string& name) { valueMap_.erase(name); }
...
const Value& ExecutionContext::getValue(const std::string& name) const { return getResult(name).value(); }
