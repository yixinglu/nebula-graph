...
class ValidateContext final {
 public:
  ...
  bool hasSpace(const std::string& spaceName) const { return createSpaces_.find(spaceName) != createSpaces_.end(); }
  ...
  void addSchema(const std::string& name, const std::shared_ptr<const meta::NebulaSchemaProvider>& schema) {
 ...
 private:
  ...
  using Schemas = std::unordered_map<std::string, std::shared_ptr<const meta::NebulaSchemaProvider>>;
  Schemas schemas_;
