...
const Result& Result::EmptyResult() {
  static Result kEmptyResult = ResultBuilder().value(Value()).iter(Iterator::Kind::kDefault).finish();
  return kEmptyResult;
}
...
ResultBuilder& ResultBuilder::iter(Iterator::Kind kind) {
  DCHECK(kind == Iterator::Kind::kDefault || core_.value) << "Must set value when creating non-default iterator";
  switch (kind) {
