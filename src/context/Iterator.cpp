...
namespace nebula {
namespace graph {
GetNeighborsIter::GetNeighborsIter(std::shared_ptr<Value> value) : Iterator(value, Kind::kGetNeighbors) {
  if (value == nullptr) {
    return;
  }
...
void GetNeighborsIter::goToFirstEdge() {
  ...
  for (currentDs_ = dsIndices_.begin(); currentDs_ < dsIndices_.end(); ++currentDs_) {
    ...
    for (currentRow_ = currentDs_->ds->rows.begin(); currentRow_ < currentDs_->ds->rows.end(); ++currentRow_) {
...
Status GetNeighborsIter::buildPropIndex(const std::string& props, size_t columnId, bool isEdge, DataSetIndex* dsIndex) {
...
void SequentialIter::init(std::vector<std::unique_ptr<Iterator>>&& iterators) {
  ...
  for (auto& iter : iterators) {
    ...
    ds.rows.insert(ds.rows.end(), std::make_move_iterator(inputIter->begin()),
...
const Value& SequentialIter::getColumn(int32_t index) const { return getColumnByIndex(index, iter_); }
...
Value PropIter::getEdge() const {
  ...
  for (auto& edgeProp : edgePropsMap) {
    ...
    for (auto& propIndex : edgeProp.second) {
      if (propIndex.first == kSrc || propIndex.first == kDst || propIndex.first == kType || propIndex.first == kRank) {
        continue;
      }
