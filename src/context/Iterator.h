...
class Iterator {
 public:
  ...
  bool isJoinIter() const { return kind_ == Kind::kJoin; }
...
class GetNeighborsIter final : public Iterator {
 ...
 private:
  ...
  Status buildPropIndex(const std::string& props, size_t columnId, bool isEdge, DataSetIndex* dsIndex);
...
class PropIter final : public SequentialIter {
 public:
  ...
  const std::unordered_map<std::string, size_t>& getColIndices() const { return dsIndex_.colIndices; }
  ...
  const Value& getTagProp(const std::string& tag, const std::string& prop) const override { return getProp(tag, prop); }
