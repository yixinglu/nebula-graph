...
const Value& QueryExpressionContext::getVersionedVar(const std::string& var, int64_t version) const {
  if (ectx_ == nullptr) {
    return Value::kEmpty;
  }
...
const Value& QueryExpressionContext::getVarProp(const std::string& var, const std::string& prop) const {
  UNUSED(var);
  if (iter_ == nullptr) {
    return Value::kEmpty;
  }
...
const Value& QueryExpressionContext::getDstProp(const std::string& tag, const std::string& prop) const {
  if (iter_ == nullptr) {
    return Value::kEmpty;
  }
  return iter_->getTagProp(tag, prop);
